import { a as BridgeLink, t as AppLayout_default } from './AppLayout-i11md3xV.js'
    ...
    setup(__props) {
        ...
        return (_ctx, _push, _parent, _attrs) => {
            _push(
                ...
                )}><div class="text-6xl font-bold text-slate-300">${ssrInterpolate(__props.status)}</div><h1 class="mt-2 text-2xl font-semibold">${ssrInterpolate(unref(title))}</h1><p class="mt-2 text-slate-500" data-testid="error-message">${ssrInterpolate(__props.message)}</p><p class="mt-6 text-sm">`,
            )
            _push(
                ssrRenderComponent(
                    unref(BridgeLink),

...
- Methods: `submit(method, url, options)`, `get/post/put/patch/delete`, `reset(...keys)`, `resetAndClearErrors(...keys)`, `setError`, `clearErrors`, `transform(fn)`, `setDefaults(values?)`, `dontRemember(...keys)`, `cancel()`, `validate(...)` (§15).

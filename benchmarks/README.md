...
Options: `BENCH_URL` (benchmark an existing server, e.g. FPM or FrankenPHP), `BENCH_TOKEN`, `BENCH_DURATION`, `BENCH_CONNECTIONS`, `BENCH_STREAMS`, `BENCH_WORKERS`, `BRIDGE_STREAM_DRIVER=redis`, `BENCH_DEBUG=1`. Requires `playground/.env.e2e` (generated by one E2E run). The Inertia baseline app from the plan is not built yet, so results compare Bridge modes only.

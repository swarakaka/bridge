...
**Post-1.0 features (2026-09-23/24),** each designed in PLAN before it was built and recorded in the "Post-1.0" deviations: JSON-mode client (`useJson`), the Vite plugin (`@swarakaka/bridge-vite`), flat form fields and form helpers, form-shaped JSON requests (`useJsonForm`, §14.1), `<BridgeForm>` (§14.2), optimistic updates (§14.3), Precognition on forms (§15.1), visit options and cache tags, the 2.0 facade layout (`Bridge\Bridge` facade, `BridgeManager` service), history encryption (§23.1), load when visible (§13.1), polling (§11.1), once props (§13.2), merge variants (§12.1), combining hints (§13.3), infinite scroll (§13.4) and watched props (§20.6, 2026-09-25). They are unreleased: the pending changesets include a major bump (the facade change), so the next release is 2.0.0. The React adapter has the same features as Vue and is still labelled experimental.
...
- Late restore (PLAN §23.1): `Router.init()` decrypts the sealed entry a full reload replaced and emits `restore` (`{ values }`); remember hooks apply values when unchanged since mount (baseline taken at mount, after chained setters like `dontRemember()`).
- Infinite scroll invalidations go through `router.handleInvalidation(prop, handler)`: the controller re-fetches the loaded range with `router.request({ only })` and applies it with `patchProps`.
...
## Watched props notes (PLAN §20.6)

- Server: `->watch()` on `PropHint` (`Bridge::watch()` = `Props\Watch`), tags from `Stream\WatchTags` (`bridge.watch.tags`: `table` or `class`, `bridgeTag()` overrides), `meta.watch` from `PropResolver::recordWatch` (not in JSON mode). Changes go through the `Stream\WatchChanges` singleton: recorded after commit, flushed on `terminating`, `JobProcessed`/`JobExceptionOccurred` and `CommandFinished`. It reads the container at call time (Octane), so never inject the app into it. Tests that publish outside a request call `app(WatchChanges::class)->flush()`. Artisan test calls only fire `CommandFinished` after `rerouteSymfonyCommandEvents()`, before the Artisan application exists.
- `X-Bridge-Client`: `RequestManager` numbers every request (`ClientIdentity`, a new random token per client), and the server echoes `sha256(token)[0..16].seq` as `client`. The router skips own-change reloads only for props delivered by that request or by one started after it settled (`deliveries` map). Anything that writes props without a page response must drop the entry (`patchProps`, history restore, cache hits).
- E2E: PHP's built-in server sends a response only after the terminating callbacks ran, so a stream message about a save arrives before the redirected page. Own-change tests must save from a page that watches the record (the star button), or the race is never exercised.

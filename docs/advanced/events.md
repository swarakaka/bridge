...
| `restore`   | `{ values }`               | Remembered state arrived after the page loaded: a full reload of an [encrypted](/security/history-encryption) page, once decrypted. `useRemember` and form `remember` apply it themselves. |

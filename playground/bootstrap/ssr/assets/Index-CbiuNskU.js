...
var Index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
    ...
    setup(__props) {
        ...
        return (_ctx, _push, _parent, _attrs) => {
            _push(`<!--[-->`)
            _push(
                ssrRenderComponent(unref(BridgeHead), { title: 'Errors · Bridge' }, null, _parent),
            )
            _push(
                `<h1 class="text-2xl font-semibold">Errors in every mode</h1><p class="mt-1 text-sm text-slate-500"> Each link triggers the error on the server. Open the same URL with <code>Accept: application/json</code> (JSON demo page) to see the Laravel-native body. </p><ul class="mt-6 space-y-2 text-sm"><!--[-->`,
            )
                        ...
                        },
                        {
                            default: withCtx((_, _push, _parent, _scopeId) => {
                                if (_push) _push(` Trigger ${ssrInterpolate(c.status)}`)
                                    ...
                                    return [
                                        createTextVNode(' Trigger ' + toDisplayString(c.status), 1),
                                    ]
                            }),
                            _: 2,
                        },
...
Index_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
    ...
    ;(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add(
        'resources/js/Pages/Errors/Index.vue',
    )

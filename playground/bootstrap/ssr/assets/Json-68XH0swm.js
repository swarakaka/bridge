...
var Json_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const curl = computed(() => {
			...
			if (endpoint.value.method !== "GET") parts.push(`-X ${endpoint.value.method}`, `-H 'Content-Type: application/json'`, `-d '${body.value}'`);
			parts.push(`${typeof window === "undefined" ? "" : window.location.origin}${endpoint.value.path}`);
			return parts.join(" \\\n  ");
		});
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<!--[-->`);
			_push(ssrRenderComponent(unref(BridgeHead), { title: "JSON demo · Bridge" }, null, _parent));
			...
			ssrRenderList(__props.endpoints, (item, i) => {
				_push(`<option${ssrRenderAttr("value", i)}${ssrIncludeBooleanAttr(Array.isArray(selected.value) ? ssrLooseContain(selected.value, i) : ssrLooseEqual(selected.value, i)) ? " selected" : ""}>${ssrInterpolate(item.method)} ${ssrInterpolate(item.path)} — ${ssrInterpolate(item.label)}</option>`);
			});
			_push(`<!--]--></select></label><label class="block"><span class="font-medium">Accept</span><select class="mt-1 w-full rounded border border-slate-300 px-3 py-2" data-testid="accept"><!--[-->`);
			ssrRenderList(__props.accepts, (type) => {
				_push(`<option${ssrRenderAttr("value", type)}${ssrIncludeBooleanAttr(Array.isArray(accept.value) ? ssrLooseContain(accept.value, type) : ssrLooseEqual(accept.value, type)) ? " selected" : ""}>${ssrInterpolate(type)}</option>`);
			});
			...
			if (result.value) {
				...
				_push(`<!--]--></ul><pre class="mt-2 max-h-[28rem] overflow-auto rounded bg-slate-900 p-3 text-xs text-slate-100" data-testid="result-body">${ssrInterpolate(result.value.body)}</pre></div>`);
			} else _push(`<!---->`);
			_push(`</div><!--]-->`);
		};
	}
...
Json_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Json.vue");

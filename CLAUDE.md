...
- Merge modes: server `Bridge\Props\Merge` (`mode`, `matchOn`, copy-returning modifiers); client `combineProp`/`readMergeModes` in `pages/PageStore.ts`, applied by `setPage(..., { partial: true, merge })`. `pages/merge.ts` is the stream `prop` modes only.

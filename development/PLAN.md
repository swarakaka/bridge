...
- Deliberately not provided: `defaults()` as a method (Bridge's `defaults` is the stored values; the method is `setDefaults`), validation errors in `onError` (Bridge uses `onInvalid`, §15), `resetOnError`/`setDefaultsOnSuccess`, which belong to a form component that does not exist yet, and an app-wide `recentlySuccessful` duration (the per-form option is `recentlySuccessfulFor`).
...
### 14.3 Optimistic updates (planned, not implemented)
...
- **Page visits.** A successful non-GET visit returns a page (after the redirect), whose props replace the optimistic values as above. Because a new visit cancels the active one (§11), rapid optimistic clicks cancel each other's requests; ownership keeps the newest optimistic value on screen while the cancelled ones give up their keys. Apps that need every click to reach the server use JSON forms (below), which run beside visits.
- **JSON forms.** `JsonForm.optimistic((props, data) => ...)` patches the same page props, but a JSON response carries no page, so on success the optimistic values stay as the new displayed values once released; `onSuccess(result)` can reconcile with `router.patchProps` (a small new router method over `PageStore.replaceProps`) or the app relies on a stream invalidation or `router.reload({ only })`. Failures roll back as above. Concurrent JSON submissions from different forms each own their keys.
- **Scope of a snapshot.** Snapshots belong to the page instance they were taken on (`PageStore` state `key`). If the user navigates to another page before a request settles, its rollback and release are discarded. Optimistic values are never written to history state: `history.updatePage` receives the store's page with owned keys replaced by their snapshots, so back/forward never restores an unconfirmed value.
...
- **Tests and docs.** Core: apply and roll back on each failure kind, success replacement, two overlapping requests on one key (first fails while the second is pending; second fails after the first succeeded), a stream `prop` event during a pending update, navigation away before settlement, history state never containing optimistic values, JSON-form success keeping values. Vue/React: a like button that renders the optimistic count and the rollback. Playground: an optimistic action (for example a customer "star") with E2E covering the rollback through a forced 422. Docs: an "Optimistic updates" page under Advanced and the migration guide row.
- **Open questions.** (1) Whether JSON-form optimistic updates should target page props (proposed) or the form's own data. (2) Whether `router.patchProps` should be public (proposed) or reconciliation left to reloads. (3) Whether rapid optimistic page visits should instead queue rather than cancel, which would change the one-active-visit rule for these visits only.
---

...
| `X-Bridge-Client`    | all        | `<token>.<seq>`: a random token of at least 128 bits, base64url-encoded without padding (22 characters), created by the client for its lifetime, and the number of this request (decimal, increasing from 1). Servers that publish watch invalidations ([stream.md](stream.md) §3.2) echo it hashed as `client`; servers MUST ignore a malformed value and MUST NOT publish the token. It does not affect the response and MUST NOT appear in `Vary`. Rationale: a client must recognise its own changes on a stream it shares with its other tabs; no standard header identifies a client instance. |
...
`X-Bridge-Build` is compared, not cached against, and MUST NOT appear in `Vary`. `X-Bridge-Client` does not affect responses and MUST NOT appear in `Vary` either.

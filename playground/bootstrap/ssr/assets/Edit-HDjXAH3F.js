...
import { t as CustomerForm_default } from './CustomerForm-C37-A9EG.js'
    ...
    setup(__props) {
        return (_ctx, _push, _parent, _attrs) => {
            ...
            _push(
                ssrRenderComponent(
                    unref(BridgeHead),
                    { title: `Edit ${__props.customer.name} · Bridge` },
                    null,
                    _parent,
                ),
            )
            _push(
                `<h1 class="text-2xl font-semibold">Edit ${ssrInterpolate(__props.customer.name)}</h1><div class="mt-6 max-w-lg">`,
            )
            _push(
                ssrRenderComponent(

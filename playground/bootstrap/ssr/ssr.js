        ...
        return this.router.visit(url, {
            ...
            onInvalid: (errors, error) => {
                const scoped =
                    only.length > 0
                        ? Object.fromEntries(
                              Object.entries(errors).filter(([k]) => only.includes(k)),
                          )
                        : errors
    ...
    prepare(request) {
        ...
        let url =
            request.url instanceof URL
                ...
                : new URL(request.url, currentHref())
    ...
    setPage(page, options = {}) {
        ...
        if (options.partial && previous && previous.component === page.component) {
            ...
            for (const [key, value] of Object.entries(page.props))
                props[key] = mergeKeys.has(key) ? appendProp(previous.props[key], value) : value
            this.state = {
                ...this.state,
                page: {
                    ...previous,
...
function captureScroll(doc = document) {
    ...
    const regions = Array.from(doc.querySelectorAll(`[${SCROLL_REGION_ATTRIBUTE}]`)).map((el) => [
        el.scrollLeft,
        el.scrollTop,
    ])
    return {
...
var Router = class {
    ...
    applyPage(page, visit) {
        ...
        if (current && this.d.window)
            this.d.history.saveScroll(captureScroll(this.d.window.document))
        this.d.store.setPage(page, { preserveState: visit.preserveState })
        const replace =
            visit.replace ||
            (this.d.window !== null &&
                relativeUrl(page.url) === relativeUrl(this.d.window.location.href))
        this.d.history.push(page, this.d.store.current.key, replace)
        if (!visit.preserveScroll && this.d.window)
            resetScroll(this.d.window.document, visit.url.hash)
        this.d.events.emit('navigate', {
            page,
            visit,
        })
    ...
    async loadDeferred(page) {
        const groups = page.deferred
            ? Object.values(page.deferred).filter((g) => Array.isArray(g))
            : []
            ...
            groups.map(async (group) => {
                ...
                try {
                    ...
                    if (
                        parsed.kind === 'page' &&
                        current &&
                        current.component === parsed.page.component
                    ) {
    ...
    onPopState(state) {
        ...
        if (this.d.window)
            this.visit(this.d.window.location.href, {
                replace: true,
                useCache: false,
            })
    }
    ...
    async connectFetch(target) {
        ...
        if (response.status === 401 || response.status === 403) {
            this.emitTransportError(
                `Stream refused with status ${response.status}`,
                response.status,
            )
            this.close()
            return
    ...
    handleControl(control) {
        ...
        switch (control.type) {
            ...
            case 'invalidate':
                ...
                if (apply)
                    this.deps.router.invalidate(
                        control.keys === '*' ? '*' : Array.from(control.keys),
                    )
                break
            case 'prop':
            ...
            case 'navigate':
                ...
                if (apply && isSameOrigin(control.url))
                    this.deps.router.navigate(control.url, control.replace ?? false)
                break
            case 'notification':
...
function createBridge(config = {}) {
    ...
    const initialPage =
        config.initialPage !== void 0 ? config.initialPage : doc ? readEmbeddedPage(doc) : null
    let build =
        config.build !== void 0
            ? config.build
...
function getBridge() {
    if (!current)
        throw new Error(
            'Bridge has not been created. Call createBridge() or createBridgeApp() first.',
        )
    return current
}
...
function withLayouts(component, node) {
    ...
    return (Array.isArray(layout) ? layout : [layout]).reduceRight(
        (child, Layout) => h(Layout, null, () => child),
        node,
    )
}
...
var pages = /* #__PURE__ */ Object.assign({
    './Pages/Auth/Login.vue': () => import('./assets/Login-BRmuez2D.js'),
    './Pages/Auth/Tokens.vue': () => import('./assets/Tokens-CBBSBPvo.js'),
    './Pages/Customers/Create.vue': () => import('./assets/Create-CeaZzEPs.js'),
    './Pages/Customers/Edit.vue': () => import('./assets/Edit-HDjXAH3F.js'),
    './Pages/Customers/Index.vue': () => import('./assets/Index-BoXHdChf.js'),
    './Pages/Customers/Show.vue': () => import('./assets/Show-BpWmuufz.js'),
    './Pages/Dashboard.vue': () => import('./assets/Dashboard-DbI1dE_O.js'),
    './Pages/Errors/Error.vue': () => import('./assets/Error-CyenXTiC.js'),
    './Pages/Errors/Index.vue': () => import('./assets/Index-CbiuNskU.js'),
    './Pages/Json.vue': () => import('./assets/Json-Bg36nhJ-.js'),
    './Pages/Realtime.vue': () => import('./assets/Realtime-CpHrBAii.js'),
})
createSsrServer({
    render: createSsrRenderer({
        resolve: (name) => {

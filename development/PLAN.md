...
### 13.1 Load when visible (planned, 2026-09-24)
...
- **Tests and docs.** Vue and React tests with a stubbed `IntersectionObserver`: no request before intersection, one coalesced request for two widgets, `buffer` passed as `rootMargin`, no request when keys are present, `always` re-entry, fallback during SSR render, load-on-mount fallback. Core needs no new module. Playground: `Customers/Show` gains an `activity` lazy prop (recent stream events for the customer, or generated entries) rendered inside `<WhenVisible>` below the fold, with an E2E spec that scrolls and asserts the single `X-Bridge-Only: activity` request. Docs: `docs/data/load-when-visible.md`, linked from lazy and deferred props.

...
### 14.1 Form-shaped JSON requests (planned, not implemented)

`useForm` always submits as a page visit, and `useJson` (§10.2–10.3) is a request handle without field state: no `defaults`, `isDirty`, `reset`, `transform`, `validate` or `remember`. A form that talks to JSON mode needs both. Design:

- **Core: one field-state base, two transports.** Extract `FormState<T>` from `Form<T>`: `data`, `defaults`, `errors`, `allErrors`, `processing`, `progress`, `wasSuccessful`, `recentlySuccessful`, `validating`, `lastError`, `isDirty`, `hasErrors`, `setData`, `transform`, `setDefaults`, `reset`, `resetAndClearErrors`, `setError`, `clearErrors`, `dontRemember`, `rememberable`, `get/post/put/patch/delete`, and the success bookkeeping (`resetOnSuccess`, defaults update, `recentlySuccessful` timer). Subclasses implement `submit` and `cancel`. `Form<T>` keeps submitting through `router.visit` and its public API is unchanged. New `JsonForm<T, R = unknown>` submits through `JsonClient` (`Accept: application/json`, the same routes, envelope unwrapped as in `JsonRequest`).
- **Response state lives beside the fields, not in `data`.** On a form `data` is the field values, so `JsonForm` exposes the last success as `result: R | null`, plus `meta`, `httpStatus` and `message`, mirroring `JsonRequest`'s names otherwise. Callbacks: `onBefore` (return `false` to skip), `onStart`, `onProgress`, `onSuccess(result, meta)`, `onInvalid(errors, message)`, `onError(error)`, `onException(error)`, `onCancel`, `onFinish(outcome)`. Like `useJson`, a `419`/`401` never reloads or redirects; the caller decides. One request in flight per form; a new submit cancels the previous one.
- **Precognition over JSON.** `validate()` moves to the base with a transport hook. `Form` keeps `router.request()`; `JsonForm` sends the same `Precognition` headers through `JsonClient`, where a 422 is Laravel-native `{message, errors}` and a 204 clears the scoped errors.
- **Adapters.** New `useJsonForm(initial, options)` and `useJsonForm('key', initial, options)` in Vue and React, returning the same shapes as `useForm` (Vue: the flat-field proxy, with reserved names computed from the `JsonForm` instance, so `result`, `meta`, `httpStatus` and `message` are reserved too; React: `useHandle` with the quiet methods). `remember`/`dontRemember` behave as in `useForm`. `useJson()` stays as the lightweight handle for reads and one-off calls.
...
- **Unchanged:** the protocol, the Laravel package (JSON mode already returns Laravel-native 422s), and the page cache rule (JSON-mode mutations already clear it, §11).
- **Out of scope for this step:** optimistic updates, a `<BridgeForm>` component (`resetOnError`, `setDefaultsOnSuccess`, `useFormContext`), Precognition helpers (`touch`, `touched`, `valid`, `invalid`, debounce).
- **Tests and docs.** Core `JsonForm` tests (success with and without envelope, 422, other errors, exception, cancel and supersede, `transform`, multipart progress, Precognition 422/204); Vue flat fields and reserved names; React parity; the playground JSON demo switched to `useJsonForm` and covered by E2E; a "Forms over JSON" section in `docs/beyond/json-mode.md`. Additive: a minor version.

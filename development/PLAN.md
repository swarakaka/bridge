...
- **Resets.** A visit that replaces the prop (a search, a filter, an invalidation reload) resets both ends from the new `meta.scroll`, because it carries a fresh page and no `merge`.
...
- **Not included.** Virtualized rendering (windowing very long lists), bidirectional URL ranges (`?from=2&to=5`), and keeping the loaded range through an invalidation (see open question 3).
...
- Full reloads. After a full document load the current entry is sealed and cannot be decrypted synchronously, so remembered state of an encrypted page does not survive a reload (documented).

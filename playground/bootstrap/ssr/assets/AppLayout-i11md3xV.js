                    ...
                    mergeProps(
                        {
                            ...
                            class: [
                                ...
                                colors[props.level] ?? colors.info,
                            ],
                        },
                        _attrs,
                    ),
                )}>${ssrInterpolate(props.message)} <button class="ml-3 opacity-70 hover:opacity-100" aria-label="Dismiss"> × </button></div>`,
            )
        }
    },
})
...
var StreamProvider_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
    ...
    setup(__props, { emit: __emit }) {
        const emit = __emit
        provideAppStream().on('notification', (n) =>
            emit('notification', {
                message: n.message,
                level: n.level,
                title: n.title ?? null,
...
StreamProvider_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
    const ssrContext = useSSRContext()
    ;(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add(
        'resources/js/Components/StreamProvider.vue',
    )
...
var AppLayout_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
    ...
    setup(__props) {
        ...
        return (_ctx, _push, _parent, _attrs) => {
            _push(
                `<div${ssrRenderAttrs(mergeProps({ class: 'min-h-screen bg-slate-50 text-slate-900' }, _attrs))}><a href="#main" class="sr-only focus:not-sr-only focus:absolute focus:left-2 focus:top-2 focus:z-50 focus:rounded focus:bg-white focus:px-3 focus:py-2"> Skip to content </a>`,
            )
            if (navigating.value)
                _push(
                    `<div data-testid="progress" class="fixed inset-x-0 top-0 z-50 h-0.5 animate-pulse bg-indigo-500"></div>`,
                )
            else _push(`<!---->`)
            _push(
                `<header class="border-b border-slate-200 bg-white"><div class="mx-auto flex max-w-5xl items-center justify-between px-4 py-3"><nav class="flex items-center gap-4" aria-label="Primary">`,
            )
            _push(
                ssrRenderComponent(
                    unref(BridgeLink),
                    ...
                    ssrRenderComponent(
                        unref(BridgeLink),
                        {
                            ...
                            class: [
                                ...
                                { 'bg-slate-100 font-medium': isActive(link.href) },
                            ],
            ...
            if (user.value) {
                ...
                _push(
                    `<span class="text-slate-500" data-testid="user-name">${ssrInterpolate(user.value.name)}</span>`,
                )
                _push(
                    ssrRenderComponent(
                        unref(BridgeLink),
            ...
            _push(
                `</div></div></header><main id="main" class="mx-auto max-w-5xl px-4 py-8" tabindex="-1">`,
            )
            if (user.value)
                _push(
                    ssrRenderComponent(
                        StreamProvider_default,
                        {
                            key: user.value.id,
            ...
            _push(
                `</main><div class="fixed right-4 top-16 z-50 space-y-2" data-testid="notifications" role="status" aria-live="polite"><!--[-->`,
            )
            ssrRenderList(notifications.value, (n) => {
                _push(
                    `<div class="w-72 rounded border border-slate-200 bg-white p-3 text-sm shadow-lg"${ssrRenderAttr('data-level', n.level)} data-testid="notification">`,
                )
                if (n.title)
                    _push(
                        `<div class="text-xs font-medium uppercase text-slate-500">${ssrInterpolate(n.title)}</div>`,
                    )
                else _push(`<!---->`)

...
- EventSource transport: a new `EventSource` after a refused connection resumes with the `lastEventId` query parameter, and a one-off JSON probe detects `401`/`403` so the client stops instead of retrying forever.

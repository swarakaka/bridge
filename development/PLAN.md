...
Laravel's exception handler and `Authenticate` middleware use `expectsJson()` (true for `application/json` and for `X-Requested-With: XMLHttpRequest`). Bridge's `ExceptionRenderer` runs before Laravel's default renderer and takes over for any request whose mode is `page`, `json` or `stream`, so Laravel's own heuristics never decide the shape of a Bridge *error*. For Page/JSON, the renderer answers an `AuthenticationException` with 401 and a `redirect` hint taken from the exception (`redirectGuestsTo`), falling back to `bridge.auth.login_url`.

`wantsJson()` is also true for the page media type (it matches `+json`), and application code uses it to choose between an API answer and a redirect (Fortify's responses, `verified`, `password.confirm`). A page visit is a browser navigation, so after negotiation `HandleBridgeRequests` replaces `Accept` with `text/html, application/xhtml+xml` for the rest of the pipeline (headers bag, `HTTP_ACCEPT` and Symfony's cached acceptable types) and restores it once the response is built. The client's value is in the `bridge.original_accept` attribute (`Negotiation::ORIGINAL_ACCEPT_ATTRIBUTE`). Bridge never reads the header after negotiating: `Negotiation::for()`, the representers, `PageResponse`, `RedirectBuilder` and `ExceptionRenderer` all use the stored negotiation. JSON and stream requests are not touched. Middleware that runs before `HandleBridgeRequests` (global, and priority-list route middleware such as `auth` and `throttle`) still sees the original header; it throws, and exceptions are represented per mode.
...
| `json/JsonForm`         | `FormState` submitted through `JsonClient` (§14.1): `result`, `meta`, `httpStatus`, `message`; one submission in flight; Precognition in JSON mode.                                                                                     |
...
| `pages/PageStore`       | Current `Page`, `setPage`, `patchProps`, `applyControl(event)` (invalidate/prop/navigate), subscription API; the optimistic layer (`applyOptimistic`, `settleOptimistic`, `serverPage`, §14.3).                                                                                                                            |
| `forms/FormState`       | Field state shared by every form (§14.1): data, defaults, dirty tracking, `errors`, `processing`, `progress`, `transform`, `reset`, `setError`, `clearErrors`, `dontRemember`, `validate` (Precognition through a transport hook), success bookkeeping. |
| `forms/createForm`      | `Form`: `FormState` submitted as a page visit through `router.visit`; Precognition through `router.request()`.                                                                                                                          |
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createBridgeApp({ resolve?, pages?, withApp?, setup, page?, progress?, stream? })` | Reads `#bridge-page` (or bootstraps when absent), resolves the component, mounts, provides the `Bridge` instance.                                                         |
...
| `useForm(initial)`                                               | Reactive wrapper around `Form`, fields also flat (`form.name`), §14.                                                                                                      |
...
| `<BridgeForm>`, `useFormContext()` | Form written as plain inputs, read by name; the enclosing form or `null` (§14.2). |
| `useJsonForm<T, R>(initial)`                                     | Reactive wrapper around `JsonForm`: `useForm` in JSON mode, no navigation, response in `form.result` (§14.1).                                                             |
...
- **Visit options (2026-09-24):** `preserveUrl` applies the response under the current address (store, history and `page.url`); `showProgress: false` sets `visit.showProgress` so indicators built on the events skip background visits (Bridge ships no bar, §27); `queryStringArrayFormat` writes GET arrays as `indices` (default, unchanged) or `brackets`, and a key in the data replaces the same key already in the URL; `invalidateCacheTags` flushes tagged cache entries when the visit succeeds (§18). `BridgeLink` passes all four through.
...
### 11.1 Polling (implemented, 2026-09-24)

Reload props of the current page on a timer. Streams (§20) remain the recommended way to keep a page current: they push `invalidate`/`prop` events only when data changes and cost nothing between events. Polling is for data without a publisher (a third-party status, a queue length computed on read) and for hosts where streams are not deployed. Client only; the protocol and the Laravel package are unchanged, because a poll is an ordinary partial reload.

- **Core API.** `router.poll(intervalMs, reload?: ReloadOptions | (() => ReloadOptions), options?: { autoStart?: boolean, keepAlive?: boolean, bindToPage?: boolean })` returns a handle `{ start(), stop(), active: boolean }` (`router/poll.ts`). A function is read on every tick, which lets adapters pass the latest options without restarting. Ticks default to `showProgress: false`: `ReloadOptions` gained `showProgress`, and a combined reload shows progress if any caller wants it, so indicators built on `visit.showProgress` stay hidden during polls while invalidations keep showing them. `autoStart` defaults to `true`. Each tick calls `router.reload(reload)`, so it inherits the existing rules: calls within the debounce window coalesce with other reloads (a stream invalidation and a tick send one request), a reload waits behind a user visit instead of aborting it (hardening note), and a reload whose page component changed meanwhile is cancelled.
- **No overlap.** The next tick is scheduled `intervalMs` after the previous reload settles (a `setTimeout` chain, not `setInterval`), so a slow server never receives a queue of reloads.
- **Hidden tabs.** Unless `keepAlive` is set, a poll pauses while `document.visibilityState` is `hidden` and, when the tab becomes visible, reloads at once if at least one tick was missed, then resumes the schedule. Pausing (rather than slowing down) keeps idle tabs from loading the server; `keepAlive` exists for dashboards on wall screens.
- **Lifetime.** By default a core poll is bound to the page instance it was started on (`PageStore` state `key`): showing another instance (a navigation without `preserveState`, a history restore) stops it, so a poll started from plain code can never reload a page it was not written for. The option `bindToPage: false` removes that rule. `stop()` is idempotent. No timers run without a `window` (SSR).
- **Adapters.** `usePoll(intervalMs, reload?, options?)` in Vue and React returns the handle; the poll starts after mount (`onMounted`, `useEffect`), never during setup, so SSR renders stay timer-free, and stops on unmount. Adapters pass `bindToPage: false` and rely on the component's lifetime instead, so a poll in a persistent layout (for example reloading a shared `notifications` prop) keeps running across navigations, while a poll in a page component stops when the page unmounts. Changing `intervalMs` (React dependency, Vue ref or getter) restarts the schedule.
- **Not included.** Polling in JSON mode (`useJson`/`JsonRequest` callers use their own timers), backoff after errors (a failed tick is reported through the usual `exception`/`error` events and the next tick runs on schedule), and a server-suggested interval.
- **Tests and docs.** Core `tests/poll.test.ts` with fake timers: tick scheduling after settle, coalescing with a stream `invalidate`, waiting behind a visit, stopping on page change, hidden/visible with and without `keepAlive`, catch-up reload. Vue and React: start after mount, stop on unmount, no timer during SSR render. Playground: the dashboard polls a `serverTime` prop every 3 s (shorter than first planned, so the E2E stays within its timeouts), covered by `e2e/tests/polling.spec.ts`: two consecutive updates, only `serverTime` requested, the progress indicator hidden, and no more requests after leaving the page. Docs: `docs/data/polling.md`, which opens by pointing to streams.
...
### 12.1 Merge variants (implemented, 2026-09-24)

`Bridge::merge()` appends only: arrays concatenate, objects concatenate their array members (a paginator's `data`) and take the rest from the response. Three variants cover the other list shapes. Merging stays opt-in per visit (`merge: true`, §12 step 6): visits without it, searches and invalidation reloads included, replace the prop, so no reset option is needed.

- **Server API.** `Merge` gains modifiers that return copies (shared hints outlive requests, as with `Once::fresh()`): `->append()` (the default), `->prepend()` (incoming items go first: feeds, chat history), and `->matchOn(string ...$paths)`. `Bridge::deepMerge($value)` merges objects key by key at every depth. A match path is a dotted path whose last segment is the item key and whose other segments lead to the array inside the prop: `'id'` for a prop that is a list, `'data.id'` for a paginator, `'thread.messages.id'` for a nested list. Match paths contain no commas or whitespace.
- **Client semantics** (core `pages/merge.ts`, applied by `PageStore.setPage(..., { merge: true })`):
  - append and prepend as today, mirrored for prepend; for an object, each array member is concatenated at the same end.
  - deep merge: objects merge recursively; arrays at any depth concatenate (append); scalars and type mismatches take the incoming value.
  - match on: before concatenating an array that a match path points at, an incoming item whose key equals an existing item's key replaces that item in place and is not added again. Items without the key are added. This removes duplicates when a list shifted between pages (a customer created while the user scrolled) and lets a reload refresh items already shown.
- **Wire.** Additive `meta` members, so protocol 1 is kept: `meta.merge` keeps listing append keys only (an old client still appends them), `meta.prepend` and `meta.deepMerge` list the keys in those modes, and `meta.matchOn` maps a key to its match paths (`{ "customers": ["data.id"] }`). A key appears in exactly one of the three lists. An old client replaces prepend and deep-merge keys, the safe fallback. JSON mode strips all four members, as it does `merge` today. `spec/page.md` §3 describes the modes; `page.schema.json` types the members; a new fixture `page/merge-modes.json` (a partial response with one key per mode and a match path) joins both conformance suites.
- **Visit option.** `merge: true` applies each key's mode. A per-visit override `merge: 'prepend'` prepends every merge key regardless of its mode, for lists loaded in both directions (§13.4); `merge: 'append'` likewise. `ReloadOptions` gains `merge`, so partial reloads can merge too.
- **Not included.** Removing items (a match that deletes), sorting after a merge, and matching on composite keys.
- **Tests and docs.** Laravel: resolver and document tests for each mode, match paths on the three shapes, copies from the modifiers, JSON stripping, the fixture. Core: `tests/merge.test.ts` for the three modes with and without match paths (duplicates replaced in place, unmatched items added, nested paths, type mismatches), the per-visit override, old-client behaviour via `readMergeKeys`. Playground: the customers list's "load more" uses `->matchOn('data.id')`; E2E creates a customer between two "load more" clicks and asserts no row appears twice. Docs: `docs/data/merging-props.md` gains the modes and match paths.
...
### 13.1 Load when visible (implemented, 2026-09-24)

`Deferred` loads right after mount whether or not the user ever scrolls to the widget. A `<WhenVisible>` component loads props when its element approaches the viewport. Client only: the server side is the existing `Bridge::lazy()` (excluded from full loads, resolved when named in `X-Bridge-Only`, spec `page.md` §4), so the protocol and the Laravel package are unchanged.

- **API.** `<WhenVisible :data="'comments'" | :data="['comments', 'related']" :buffer="200" :always="false" as="div" :reload="{ headers }">` with a `fallback` slot shown until every listed key is present and a default slot afterwards (the same readiness rule as `<Deferred>`). React: `<WhenVisible data={...} buffer={200} always={false} as="div" fallback={...}>`. A headless `useWhenVisible(elementRef, keys, options)` returns `{ visible, loading }` for custom markup; `trackVisibility: false` releases the observer once the keys have loaded (the component passes it). Both adapters are bindings over core `loadWhenVisible(bridge, element, options)` (`pages/whenVisible.ts`), which returns a handle with `stop()`.
- **Loading.** The component renders a wrapper element (`as`, default `div`) observed with `IntersectionObserver` (`rootMargin` of `buffer` px, default 0). When it intersects and a listed key is missing, it calls `router.reload({ only: keys, ...reload })` and marks the keys loading through `PageStore.setLoading`, so `useDeferred(key).loading` works for these keys too. The reload path gives coalescing (several widgets entering the viewport together send one request), waiting behind a user visit, and cancellation when the page changed.
- **Once or always.** By default observation stops after the first successful load, and keys already present (restored from history, or loaded by a partial reload) cause no request. `always` reloads on every entry into the viewport, for data that goes stale while off-screen; it still skips entries while a load for the same keys is in flight.
- **Environment.** SSR renders the fallback (no observer, no request) unless the keys are already in the page, so hydration matches. Without `IntersectionObserver` (very old browsers, some test environments) the component loads on mount, which degrades to `Deferred` behaviour rather than never loading.
- **Not included.** Infinite scrolling (appending pages of a `Merge` prop as the user scrolls; it needs merge on reloads and URL handling and gets its own design), and visibility-triggered prefetch of links.
- **Tests and docs.** Vue and React tests with a stubbed `IntersectionObserver`: no request before intersection, one coalesced request for two widgets, `buffer` passed as `rootMargin`, no request when keys are present, `always` re-entry, fallback during SSR render, load-on-mount fallback. Core `tests/whenVisible.test.ts` covers the shared helper. Playground: `Customers/Show` gains an `activity` lazy prop (recent stream events for the customer, or generated entries) rendered inside `<WhenVisible>` below the fold, with an E2E spec that scrolls and asserts the single `X-Bridge-Only: activity` request. Docs: `docs/data/load-when-visible.md`, linked from lazy and deferred props.

### 13.2 Once props (implemented, 2026-09-24)

Props that rarely change (plan lists, country lists, translations, feature flags) are resolved and sent on every visit today. A once prop is sent the first time and then reused by the client until it expires, so later visits skip both the server work and the payload. This needs the protocol, the server and the client.

- **Server API.** `Bridge::once(Closure|mixed $value, ?string $key = null, DateInterval|int|null $ttl = null): Once`, a new `PropHint`. `$key` names the cached value independently of the prop name, so two pages (or a shared prop and a page prop) can share one entry; it defaults to the prop name. `$ttl` is seconds or an interval; `null` means no expiry. `->fresh(bool $fresh = true)` resolves and sends the value even when the client holds it (after the server knows it changed). Once props work as shared props (`Bridge::share('plans', Bridge::once(...))`), the main use case. `Once` does not wrap `Lazy`, `Deferred`, `Always` or `Merge` in this step (the resolver refuses nested hints with an exception naming the prop).
- **Wire.** Page mode only. The page object lists once props in `meta.once`: `{ "<prop>": { "key": "<once key>", "expiresAt": <epoch ms> | null } }`, whether or not the value is in `props`. The client sends the once keys it holds and that have not expired in a new request header `X-Bridge-Once: plans,countries` (comma-separated, same syntax as `X-Bridge-Only`). For a once prop whose key is listed, the server omits the value from `props` but keeps the `meta.once` entry; otherwise it resolves the prop normally. A prop named in `X-Bridge-Only` is always resolved (an explicit reload is a refresh), and so is a prop marked `fresh`. `X-Bridge-Once` joins `Vary` for page responses. JSON mode resolves once props inline and ignores the header: JSON clients have no page store to fill from, and a mobile SDK that wants the feature can adopt the header later without a protocol change. The HTML shell never receives the header, so an embedded page always carries every value.
- **Compatibility.** Additive within protocol 1: an old client sends no `X-Bridge-Once` and so always gets values, and it ignores `meta.once` as an unknown `meta` member. `spec/page.md` gains a "Once props" section and the `once` row in the §4 hint table; `spec/headers.md` gains `X-Bridge-Once` and the `Vary` row; `page.schema.json` gives `meta` typed optional members (`merge`, `once`) while still allowing others. New fixtures: `page/once-full.json` (value sent, listed in `meta.once`) and `page/once-held.json` (value omitted, still listed), each with the request headers that produce it, asserted by the PHP conformance suite and the TS conformance tests.
- **Client store.** `pages/OnceStore` in core holds `{ key → { value, expiresAt } }` in memory for the tab. When a page is applied (visit, reload, deferred load, prefetch-cache hit, history restore) the router first stores the values of listed once props that are present, then fills listed props that are absent from the store, before `PageStore.setPage`, so components and history always see complete props. If a listed prop is absent and the store has no live value (it expired between request and response, or a prefetched page is used after the value was dropped), the router requests it with a partial reload, as it does for deferred groups. The router passes `X-Bridge-Once` on visits, reloads, deferred loads and prefetches (not on Precognition or JSON-mode requests). The page cache key does not include it: a cached page is completed from the store when it is applied, and the partial-reload fallback covers a value the store no longer has. A value the server left out is filled even if it expired after the request announced it; an expiry is only taken from a response that carried the value.
- **Invalidation.** Expired entries are dropped on read. The store is cleared on `router.clearCache()`, on 401/403/419 (like the page cache), and on a history clear (§23.1), because once values can be user-specific. `bridge.cache.clear()` clears only pages: the router also calls it after every mutation and invalidation, which must not drop once values. A stream `invalidate` naming a once prop reloads it (a partial reload with `only`), which also refreshes the stored value. A build change forces a full document load, which empties the store.
- **Not included.** Persisting once values across full page loads (`sessionStorage`/`localStorage`): a full load is rare in an SPA, and persisted values would need encryption and cross-user separation; revisit if measurements show value.
- **Tests and docs.** Laravel: resolver unit tests (sent without the header, omitted with it, `X-Bridge-Only` and `fresh` override, custom key shared by two props, TTL to `expiresAt` with a fake clock, JSON mode inline, nested-hint refusal) and conformance for the fixtures. Core: `tests/once.test.ts` (header content, fill from store, expiry, missing-value reload, prefetch-cache interplay, clearing on 401 and cache clear, history entries hold complete props). Playground: the `statuses` prop of `Customers/Create` and `Customers/Edit` becomes `Bridge::once(..., key: 'customer-statuses')`, one entry shared by both pages; E2E opens Create, then Edit, and asserts the second response omits `statuses` while the select still renders the options. Docs: `docs/data/once-props.md`.

### 13.3 Combining delivery hints (implemented, 2026-09-24)

Each hint is its own class today, and the resolver refuses a hint that resolves to another (§13.2 deviation). Two combinations are useful and have a clear meaning on the wire: a deferred prop that later merges (the first page of a feed after render, more pages on scroll) and a deferred or lazy prop that is kept once (a slow value computed once per tab). They become modifiers, not nesting.

- **Model.** A prop has one *delivery*: plain, `lazy`, `defer` (with its group) or `always`. It may add *merge* (§12.1 modes and match paths) and *once* (key, ttl, fresh). `Bridge::merge($v)` and `Bridge::once($v, ...)` stay as shorthands for a plain prop with that modifier. Modifiers chain on the delivery hints: `Bridge::defer(fn, 'feed')->merge()->matchOn('data.id')`, `Bridge::defer(fn)->once(key: 'plans', ttl: 3600)`, `Bridge::lazy(fn)->merge()`, `Bridge::lazy(fn)->once()`. Internally the hint classes share a `PropHint` base holding the modifiers; `Merge` and `Once` remain as classes (plain delivery) for existing `instanceof` checks and imports.
- **Allowed combinations.** `always` takes no modifier (it is sent on every response by definition). `once` with `merge` is refused: a held value is never sent, so there is nothing to append to, and a merged value is not a stable value to keep. Everything else composes. Refusals throw when the modifier is added, naming the combination.
- **Resolution rules** (`PropResolver`, in this order): selection (partial headers) → delivery (lazy: absent unless named; defer: absent on full loads and listed in `deferred`, resolved when named or in JSON mode) → once (a held key is left out even where the delivery would include it, except when named in `X-Bridge-Only` or `fresh`) → merge (listed in the §12.1 members whenever the value is sent).
- **What a client sees.** A held deferred-once prop is in neither `props` nor `deferred`: it is listed in `meta.once`, the client fills it, and no deferred request is made. A lazy-once prop the client holds is listed in `meta.once` on full loads and filled, so `<WhenVisible>` finds it present and loads nothing. A deferred-merge prop arrives with the deferred request like any deferred prop; later partial reloads with `merge: true` append to it. No new wire members: `spec/page.md` §4 gains a table of combinations and their effect, and a fixture `page/deferred-once-held.json` (held key: absent from `props` and `deferred`, present in `meta.once`) joins both conformance suites.
- **Client.** The deferred loader skips groups whose keys are all present after once filling (it already skips present keys). Nothing else changes: filling, merging and deferred loading already compose.
- **Not included.** `once` on `always`, modifiers on shared props other than through the hint itself, and per-group modifiers.
- **Tests and docs.** Laravel: a resolver matrix (each delivery × merge × once, full, partial and JSON), refusals, and the fixture. Core: a held deferred-once prop causes no deferred request; a deferred-merge prop appends on a later merge visit. Playground: the dashboard's `signups` chart becomes `Bridge::defer(..., 'charts')->once(key: 'signups', ttl: 300)`, and E2E asserts the second dashboard visit in the same tab sends no `charts` request. Docs: a "Combining hints" section in `docs/data/deferred-props.md`, and cross-links from merging and once props.

### 13.4 Infinite scroll (implemented, 2026-09-24)

A list that loads its next page as the user scrolls, and its previous page when the user opened a later page directly. It builds on merge variants (§12.1: prepend, match paths), the combined hints (§13.3) and the visibility helper of §13.1.

- **Server.** `Bridge::scroll($paginated, ?string $pageName = null)` is a plain prop with merge (append) whose value is a paginator, a `ResourceCollection` over one, or a cursor paginator. It adds `meta.scroll`: `{ "<prop>": { "pageName": "page", "dataPath": "data", "currentPage": 3, "previousPage": 2, "nextPage": 4 } }` (cursor paginators give cursor strings; `null` at either end). The shape is independent of how the paginator serializes, so the client never parses `links`. A match path defaults to `data.id` when every item on the page has an `id` (override with `->matchOn()`, disable with `->matchOn()` and no paths). JSON mode serializes the paginator as today and strips `meta.scroll`.
- **Client component.** `<InfiniteScroll data="customers">` (Vue and React) renders its slot and two invisible sentinels, before and after it, observed with `IntersectionObserver` (`buffer` px, default 500). When the after-sentinel is visible and `nextPage` is not null, it visits the current URL with `{ [pageName]: nextPage }`, `only: [prop]`, `merge: 'append'`, `preserveState`, `preserveScroll`, `showProgress: false`; the before-sentinel loads `previousPage` with `merge: 'prepend'` and keeps the viewport still (it records the scroll height before the update and scrolls by the difference after it). One load per direction at a time; a load's `meta.scroll` updates only its own end (a next load moves `nextPage`, a previous load moves `previousPage`).
- **URL.** After each load the address is replaced (`history.replaceState`, no new entry) with the page closest to the viewport, so reloading or sharing the address opens the list where the user was, and the before-sentinel loads upward from there. `preserveUrl` keeps the address unchanged instead (today's "load more" behaviour). Back/forward restores the whole loaded list from history like any page.
- **Options.** `buffer`, `as`, `manual` (no automatic loading: the `next`/`previous` slots receive `{ load, loading, hasMore }` to render buttons), `manualAfter: n` (automatic for n loads, then manual), `reverse` (for chat: the list starts at the bottom and older pages load at the top), `preserveUrl`, `only` (extra props to reload with each page, e.g. counts). Slots: `default`, `loading` (per end), `next`, `previous`. A headless `useInfiniteScroll(prop, options)` returns `{ loadNext, loadPrevious, loadingNext, loadingPrevious, hasNext, hasPrevious }` plus sentinel refs for custom markup.
- **Resets.** A visit that replaces the prop (a search, a filter) resets both ends from the new `meta.scroll`, because it carries a fresh page and no `merge`.
- **Invalidations (changed 2026-09-24, open question 3).** An invalidation naming the prop (stream `invalidate`, `router.invalidate()`, including `'*'`) re-fetches every loaded page instead of replacing the list with one page: the controller registers `router.handleInvalidation(prop, handler)`, requests the loaded range from its first page following each response's `nextPage` (through `router.request()`, outside the visit pipeline), combines the pages with the prop's match paths, and applies the result once with `router.patchProps()`. The address and the upper end stay; the lower end and the page count follow the server. An invalidation during a load runs after it.
- **Environment.** SSR renders the items and the `next`/`previous` slots in their manual form, so the first paint works without JavaScript and hydrates to the same markup. Without `IntersectionObserver` the component behaves as `manual`. Keyboard and assistive technology users get the manual buttons as well: the component always renders them, visually hidden while automatic loading is on, and announces loaded pages through an `aria-live` region.
- **Not included.** Virtualized rendering (windowing very long lists), bidirectional URL ranges (`?from=2&to=5`), and limits on how many pages an invalidation re-fetches.
- **Tests and docs.** Laravel: `meta.scroll` for length-aware, simple and cursor paginators and resource collections, the default match path, JSON stripping, a fixture `page/scroll.json`. Core: a `scroll` helper in `pages/` shared by both adapters (loading each end, replacing the address, updating only its own end, reset on replace), with the viewport adjustment tested against a stubbed layout. Vue and React: sentinels, manual and `manualAfter`, reverse, SSR markup, no-observer fallback. Playground: the customers list replaces its "load more" button with `<InfiniteScroll data="customers">` (the button stays as the manual slot), keeping the pagination links for keyboard users; E2E scrolls to load pages 2 and 3, asserts the address says page 3, reloads, scrolls up and sees page 2 prepended without the viewport jumping. Docs: `docs/data/infinite-scroll.md`.
...
- Methods: `submit(method, url, options)`, `get/post/put/patch/delete`, `setData(values | key, value | callback)`, `reset(...keys)`, `resetAndClearErrors(...keys)`, `setError`, `clearErrors`, `transform(fn)`, `setDefaults(values?)`, `dontRemember(...keys)`, `cancel()`, `validate(...)` (§15).
...
- `remember`: form data is stored in history state so back-navigation restores it. The key is the `remember` option or the first argument (`useForm('login', data)`). `dontRemember(...keys)` keeps top-level fields (passwords, tokens) out of it: adapters write and restore `form.rememberable()`, the data without those fields.
- Field access (Vue adapter): values live in `data`, and `useForm` also exposes each field on the form (`form.name` is `form.data.name`, reads and writes) through a proxy over the reactive form. Field names that are `Form` members (instance, prototype chain and `Object.prototype`, computed from the instance) are refused with an error at `useForm()` time. The core `Form` and the React adapter keep `data`/`setData` only.
- Deliberately not provided: `defaults()` as a method (Bridge's `defaults` is the stored values; the method is `setDefaults`) and validation errors in `onError` (Bridge uses `onInvalid`, §15). Reset behaviour is `resetOnSuccess` (all or listed fields), `resetOnError` and `setDefaultsOnSuccess`, per submit, per form, or for every form through `BridgeConfig.forms`, which also sets the app-wide `recentlySuccessfulFor`.

### 14.1 Form-shaped JSON requests

`useForm` submits as a page visit, and `useJson` (§10.2–10.3) is a request handle without field state: no `defaults`, `isDirty`, `reset`, `transform`, `validate` or `remember`. `useJsonForm` is a form that talks to JSON mode.

- **Core: one field-state base, two transports.** `FormState<T, K>` (`forms/FormState.ts`) holds `data`, `defaults`, `errors`, `allErrors`, `processing`, `progress`, `wasSuccessful`, `recentlySuccessful`, `validating`, `lastError`, `isDirty`, `hasErrors`, `setData`, `transform`, `setDefaults`, `reset`, `resetAndClearErrors`, `setError`, `clearErrors`, `dontRemember`, `rememberable`, `get/post/put/patch/delete`, `validate`, and the submission bookkeeping (`beginSubmit`, `endSubmit`, `completeSuccess`: `resetOnSuccess`, defaults update, `recentlySuccessful` timer). `K extends FormTransport` names the transport's submit options, outcome, error and 422 detail types. Subclasses implement `submit`, `cancel`, `precognition` and `recordInvalid`. `Form<T>` (`PageFormTransport`) submits through `router.visit`; its public API is unchanged. `JsonForm<T, R = unknown>` (`JsonFormTransport<R>`) submits through `JsonClient` (`Accept: application/json`, the same routes, envelope unwrapped as in `JsonRequest`). `bridge.jsonForm(initial, options)` builds one; `options.headers` go on every request.
- **Response state lives beside the fields, not in `data`.** `data` is the field values, so `JsonForm` exposes the last success as `result: R | null` with `meta`, and `httpStatus` and `message` for the last response. Callbacks: `onBefore` (return `false` to skip), `onStart`, `onProgress`, `onSuccess(result, meta)`, `onInvalid(errors, message)`, `onError(error)`, `onException(error)`, `onCancel`, `onFinish(outcome)`. A `419`/`401` never reloads or redirects; the caller decides. One submission in flight: a new submit or `cancel()` aborts the previous one, which resolves `{ status: 'cancelled' }` and calls its `onCancel` and `onFinish`, leaving the form state to the newer request.
- **Precognition over JSON.** `validate()` lives in the base and calls the transport's `precognition` hook, which classifies the answer (`invalid`, `error`, `passed`, `unexpected`, `cancelled`). `Form` keeps `router.request()`. `JsonForm` sends the same `Precognition` headers through `JsonClient`, where a 422 is Laravel-native `{message, errors}` and a 204 passes; the request passes `mutation: false`, a new `JsonRequestOptions` flag, so a validation does not clear the page cache the way JSON-mode mutations do (§11).
- **Adapters.** `useJsonForm(initial, options)` and `useJsonForm('key', initial, options)` in Vue and React return the same shapes as `useForm`. Vue: the flat-field proxy and the reserved-name check are shared with `useForm` (`setupForm`), with names computed from the `JsonForm` instance, so `result`, `meta`, `httpStatus` and `message` are reserved too; the in-flight request is cancelled when the component scope is disposed (`cancelOnDispose: false` opts out, as for `useJson`). React: `useHandle` with the quiet methods; the request is cancelled on unmount. `remember`/`dontRemember` behave as in `useForm` and share its `form:` history-state namespace. `useJson()` stays as the lightweight handle for reads and one-off calls.
- **Why a new name.** `useJson(options)` cannot also take initial values: `useJson({ headers })` and `useJson({ name: '' })` are both plain objects. `useJsonForm` follows the mode naming used by `useJson`/`usePage`/`useStream` (see the §10.2 deviation on the `useJson` name); the migration guide maps the equivalent hook to `useJsonForm`.
- **Unchanged:** the protocol and the Laravel package (JSON mode already returns Laravel-native 422s and answers Precognition).
- **Not included:** optimistic updates, a `<BridgeForm>` component (`resetOnError`, `setDefaultsOnSuccess`, `useFormContext`), Precognition helpers (`touch`, `touched`, `valid`, `invalid`, debounce).
- **Tests and docs.** Core `JsonForm` tests (`packages/core/tests/jsonForm.test.ts`: success with and without envelope, 422, other errors including 419/401, exception, cancel and supersede, `transform`, `onBefore`, multipart progress and method spoofing, Precognition 422/204 and the cache rule); Vue flat fields, 422, remember with `dontRemember`, cancel on dispose and reserved names; React parity and cancel on unmount; the playground JSON demo has a `useJsonForm` card (create a customer, live validation on blur), covered by `e2e/tests/json-demo.spec.ts`; the "Forms over JSON" section in `docs/beyond/json-mode.md`.

### 14.2 Form component and form context

A `<BridgeForm>` component for forms written as plain HTML inputs, and `useFormContext()` for components nested inside one. It adds no new form state: it is a `FormState` (§14, §14.1, §15.1) whose values come from the DOM instead of `v-model`/`setData`.

- **Values from the DOM.** Inputs are uncontrolled and identified by `name`. On submit, and on every `input`/`change` event inside the element, the component reads `new FormData(formElement)` and turns it into an object with a new core helper `formDataToObject` (`http/formData.ts`, the inverse of `objectToFormData`), then calls `form.setData(...)` so `isDirty`, `validate`, `remember` and `transform` work unchanged. Name syntax: `a`, `a[b]` and `a.b` (nested), `a[]` (append to a list), `a[0][b]` (indexed), `a\.b` (a literal dot); a repeated name without `[]` keeps the last value; `File` values are kept; an unchecked checkbox is absent, and a checkbox without a `value` sends `"on"` (documented: give checkboxes `value="1"`). Initial values come from the markup (`value`/`defaultValue`, `checked`/`defaultChecked`, `selected`) and are read once after mount as the defaults.
- **Reset and defaults in the DOM.** The DOM follows the form: whenever `form.data` or `form.defaults` changes (`reset`, `resetOnSuccess`/`resetOnError`, restored `remember` state, the new defaults after a success), the fields' values and their `defaultValue`/`defaultChecked`/`defaultSelected` are written back (`writeFormElement`, which touches only fields that differ so typing is undisturbed), so a native `<button type="reset">` agrees with `form.reset()`. In Vue a re-render re-applies the markup's `value`/`checked` to both the property and the attribute (Vue 3.5), so the component also writes defaults then values after every update; in React inputs are uncontrolled (`defaultValue`), which avoids that.
- **Props.** `action`, `method` (default `post`; bound with `withPrecognition(method, action)` so `submit()` and `validate(field)` need no arguments), `json` (submit as a `JsonForm` in JSON mode, §14.1, instead of a page visit), `transform`, `headers`, `options` (visit options: `preserveScroll`, `preserveState`, `preserveUrl`, `replace`, `only`, `except`, `showProgress`, `queryStringArrayFormat`, `invalidateCacheTags`), `resetOnSuccess` and `resetOnError` (`true` or a list of fields; `resetOnError` applies to `invalid` and `error` outcomes), `setDefaultsOnSuccess` (default `true`, Bridge's `useForm` behaviour; `false` keeps the old defaults), `disableWhileProcessing` (sets `inert` on the element while `processing`), `cancelOnUnmount` (default `true`), `validationTimeout`, `validateFiles`, `remember` (history-state key, with `dontRemember` through the slot). Events/callbacks mirror the visit callbacks: `before`, `start`, `progress`, `success`, `invalid`, `error`, `exception`, `cancel`, `finish` (Vue emits, React `on*` props). No `errorBag` (each form owns its errors, §15) and no `withAllErrors` (`allErrors` exists).
- **Rendering.** A real `<form>` element with `action` and, for `get`/`post`, `method`; other methods render `method="post"` and no spoofing field. Submission always goes through the client (`preventDefault`); working without JavaScript is not a goal of this step because a classic post would also need a CSRF `_token` field, which Bridge does not expose today (see open questions). Other attributes pass through. SSR renders the same markup; nothing reads the DOM until mounted.
- **Slot props / render prop.** The reactive form itself: every `FormState` member (`errors`, `processing`, `progress`, `isDirty`, `wasSuccessful`, `recentlySuccessful`, `validating`, `submit`, `reset`, `resetAndClearErrors`, `setError`, `clearErrors`, `setDefaults`, `validate`, `touch`, `touched`, `valid`, `invalid`, `cancel`), plus `result`, `meta` and `message` with `json`. Vue: default scoped slot (`<BridgeForm v-slot="form">`). React: `children` may be a function of the form, or plain nodes that use `useFormContext()`.
- **Imperative access.** Vue `expose`s an object of getters over every form member (live state, bound methods), so a template ref gives `formRef.value.submit()`, `formRef.value.isDirty`; exposing the reactive form directly would let Vue mark it raw. React forwards a ref to the form (`useImperativeHandle`). The ref is the form, not the `<form>` element; no `form.element` member is added. `submit()` on the slot, ref or context carries the component's `options`, `headers`, reset props and events, like the native submit.
- **`useFormContext()`.** Vue `provide`/`inject`, React context: returns the nearest enclosing `<BridgeForm>`'s form, or `null` outside one (not an error, so shared inputs can work inside and outside forms). Typed as `BridgeFormInstance<T> | null` (a `Form<T>` with the `JsonForm` response members optional). React provides a new `{ form }` object on every render: the form object is stable, so consumers would otherwise not re-render.
- **Core changes.** `formDataToObject` and `parseFieldName` (`http/formData.ts`) and `readFormElement`, `writeFormElement`, `fileFieldNames` (`forms/dom.ts`), shared by both adapters. `resetOnError` (`true` or fields), field lists for `resetOnSuccess`, and `setDefaultsOnSuccess` became `SubmitResetOptions` on every form (per form and per submit) rather than component-only logic: listed fields must be reset before the new defaults are recorded, which only the success bookkeeping can do, and `useForm`/`useJsonForm` get them too.
- **Why a component on top of `useForm`.** Server-rendered or long HTML forms (many inputs, dynamic rows named `items[3][qty]`) are shorter without a `v-model` per field, and the DOM already holds the values. The component is a thin binding: every behaviour stays in `FormState`, so the two styles cannot drift.
- **Tests and docs.** Core `tests/formElement.test.ts` (name parsing, `formDataToObject` and its round trip with `objectToFormData`, reading and writing a form element including defaults and a native reset, reset options). Vue `tests/bridgeForm.test.ts` and React `parity.test.tsx` (reading, typing, submit body and events, errors in the slot, reset writing back, field-list resets, `inert`, `json`, context inside and outside, refs, bound `validate`, cancel on unmount). The playground login page uses `<BridgeForm>` with `resetOnError: ['password']`, so every E2E sign-in goes through it under SSR, and `auth-and-errors.spec.ts` checks the cleared password. Docs: `docs/basics/form-component.md` and the migration guide.
- **Decisions (2026-09-24, maintainer).** The name is `BridgeForm`; `setDefaultsOnSuccess` defaults to `true`; submission without JavaScript stays unsupported. Initial values come from `value`/`checked`/`selected` in Vue and `defaultValue`/`defaultChecked` in React; the Vue component writes them back after re-renders, as described above.

### 14.3 Optimistic updates

Show the expected result of a mutation before the server answers, and undo it when the server refuses. This touches the page store (§12) and the router (§11) as well as forms.

- **API.** A visit option `optimistic: (props) => Partial<props>`; `router.optimistic(fn)` returning the verb helpers (`router.optimistic(fn).post(url)`) as sugar for that option; `form.optimistic(fn)` on every `FormState`, a one-shot for the next submission, with `fn(props, data)`; and an `optimistic` prop on `<BridgeForm>` (§14.2). The callback returns a partial object of top-level page props, shallow-merged into the current props. A dotted key (`'post.likes'`) is not supported in this step; return the whole top-level prop.
- **Snapshots and ownership.** Applying an update records, per top-level key it returns, the value before the first pending optimistic change to that key (a snapshot) and which pending requests touch the key. On failure (`invalid`, `error`, `exception`, or `cancelled`, including a visit cancelled by a newer one), a key is reverted to its snapshot only if no other pending request still touches it; otherwise the failed request just gives up its ownership. On success the request gives up ownership too. When the last owner of a key settles, the key is released.
- **Server data while a key is owned.** A page from a successful visit, a partial reload, a deferred-prop load or a stream `prop` control event updates the snapshot of an owned key instead of the displayed value; the displayed value stays optimistic until the key is released, and then shows the latest server value (or, if the last owner succeeded with a page, the value from that page). Keys that are not owned are applied as today. Stream `invalidate` still reloads; its result goes through the same rule.
- **Page visits.** A successful visit that applies a page settles its update right after the page is applied (`server`), so the page's props show; a `204` success keeps the optimistic values (`keep`), since it carries no page; every other end (`invalid`, `error`, `exception`, `cancelled`, `redirected`) settles with `server`. Because a new visit cancels the active one (§11), rapid optimistic clicks cancel each other's requests; ownership keeps the newest optimistic value on screen while the cancelled ones give up their keys. Apps that need every click to reach the server use JSON forms (below), which run beside visits.
- **JSON forms.** `JsonForm.optimistic((props, data) => ...)` and the `optimistic` submit option patch the same page props (the bridge passes its `PageStore` to `JsonForm` as an `OptimisticTarget`). A JSON response carries no page, so on success the optimistic values stay as the new displayed values once released (`keep`); `onSuccess(result)` can reconcile with the public `router.patchProps(patch)` (server data: held props change when released, and history stores the new values) or the app relies on a stream invalidation or `router.reload({ only })`. Failures, cancellations and superseded submissions roll back. Concurrent JSON submissions from different forms each own their keys.
- **Scope of a snapshot.** Snapshots belong to the page instance they were taken on (`PageStore` state `key`): showing another instance (a navigation without `preserveState`, a history restore) drops them, and a later settle for them is ignored. Optimistic values are never written to history state: the router writes `store.serverPage` (the page with held props replaced by their snapshots) wherever it used the store's page, and full page swaps push the response itself.
- **Events.** No new router events; the visit's own `success`/`invalid`/`error`/`cancel` drive settlement. Vue and React re-render through the existing store subscription.
- **Out of scope.** Optimistic changes to form data (the JSON form's own `data`) rather than page props; nested-key snapshots; retry queues for offline use.
- **Tests and docs.** Core `tests/optimistic.test.ts` (store: rollback and keep, overlapping owners, server data while held including a stream `prop` event and `patchProps`, navigation away; visits: rollback after 422, 500 and a network failure, success replacement, a newer visit cancelling an older one, history never holding optimistic values; forms: one-shot `form.optimistic` with the form data, JSON form keep and rollback). Vue and React: a like counter and `<BridgeForm optimistic>` rendering the optimistic value and the rollback. Playground: a customer star (`POST /customers/{id}/star`, refused with a 422 for locked customers), with `e2e/tests/optimistic.spec.ts` holding the request to observe the optimistic state, then the confirmation and the rollback. Docs: `docs/advanced/optimistic-updates.md`, the visit option, the form component prop, the migration guide.
- **Decisions (2026-09-24, maintainer).** JSON-form updates target page props; `router.patchProps` is public; rapid optimistic page visits cancel each other (the one-active-visit rule is unchanged).
---
...
### 15.1 Precognition on forms

Every form (`FormState`, so page and JSON forms alike) validates through Precognition in two styles:

- **Bound endpoint.** `useForm(method, url, data)` / `useJsonForm(method, url, data)` in both adapters, or `form.withPrecognition(method, url)`, stores an endpoint used by `validate(...)` and by `submit(options?)` without a method and URL. `validate('email')`, `validate(['a', 'b'])` and `validate({ only, ...options })` validate those fields; `validate()` validates the touched fields and sends nothing when none are. Calls are debounced (`setValidationTimeout(ms)`, default 1500, `0` disables): the first call is sent at once, later calls within the window are combined, fields merged, into one request at its end, and every combined call resolves with its outcome. `File`/`Blob`/`FileList` values are removed from the payload and fields holding them are dropped from `Precognition-Validate-Only` (a request left with no fields is not sent) unless `validateFiles()` was called, so live validation never uploads.
- **Explicit endpoint.** `validate(method, url, fields?, options?)` keeps its Phase 4 behaviour: sent at once, the whole data including files, and every rule when no field is given. Keeping it unchanged makes the bound style additive.
- **State.** `touch(...fields)` (all top-level fields without arguments) and `touched(field?)` (any field without an argument); `validate(field)` does not touch. `valid(field)` is true once a Precognition answer covered the field and it has no error; `invalid(field)` is true while it has an error. `reset()` forgets touched and validated state for the fields it resets. Options add `onBefore` (return `false` to skip) and `only` to `onInvalid`, `onError`, `onSuccess`, `onFinish` and `headers`.
- **Names.** The validation callback stays `onInvalid` (Bridge's name everywhere, §15) rather than a second alias; `allErrors` already holds every message per field, so there is no `withAllErrors()`. The state lives in one private `precog` member so the Vue flat-field proxy reserves one extra name, plus the public methods. React exempts `touched`, `valid`, `invalid` and the chainable setters from re-rendering, so they can run during render.
...
- `router.prefetch(url, { only, cacheTags })` populates `PageCache` with `{ page, fetchedAt, tags }`; `router.flushByCacheTags(tags)` removes entries with any of the tags, and a page stored again without tags keeps the entry's tags. TTL default 30 s; `staleWhileRevalidate` default 30 s more (serve cached, re-fetch in background, patch when it arrives).
...
### 23.1 History encryption (implemented, 2026-09-24)

**Problem.** Every page swap stores the whole page object, props included, and remembered component state (`useRemember`, form data from `remember`) in `history.state` (§11, `router/History.ts`). The browser keeps that state with the session history, so after a user logs out, back/forward restores their pages without a request (`restoreFromHistory: true`), and the data survives in tab-restore and session-restore files. The server-side caching rules (§24: `no-store` for authenticated shells) do not reach this copy. History encryption makes stored entries unreadable once the application says so.

**Server.**

- `bridge.history.encrypt` (default `false`) encrypts every page. Per response: `Bridge::encryptHistory(bool $encrypt = true)` (request-scoped, for controllers and middleware), `PageResponse::encryptHistory()`, and a route middleware alias `bridge.encrypt-history` for route groups (the common setup: every authenticated route).
- `Bridge::clearHistory()` asks the client to make all earlier encrypted entries unreadable. The flag is set on the request; when the response is built, `HandleBridgeRequests` puts it in the response's page (`meta.clearHistory: true`), or, when the response is a redirect and a session exists, stores it in the session (`bridge.clear_history`) so the next page carries it and consumes it. Writing at response time, after the controller ran, matters because logout controllers invalidate the session before redirecting, which would drop a value written earlier.
- `bridge.history.clear_on_logout` (default `true`) listens to `Illuminate\Auth\Events\Logout` and calls `clearHistory()`, so Fortify and hand-written logout actions work without code.
- Wire: `meta.encryptHistory: true` and `meta.clearHistory: true` on the page object, embedded shells included. Additive within protocol 1 (unknown `meta` members are ignored; an old client stores plaintext as today). `spec/page.md` gains a "History" section defining both members and the client obligations below as SHOULDs; `page.schema.json` types them; fixtures `page/encrypt-history.json` and `page/clear-history.json` with conformance tests (including the flash across a logout redirect).

**Client.**

- `router/HistoryCipher` in core: AES-GCM with a 256-bit key from Web Crypto, a fresh 96-bit IV per write. The raw key is kept in `sessionStorage` (per tab, gone when the tab's session ends) together with a key id. A `CryptoKey` is not used as the stored form because `sessionStorage` holds strings only.
- What is sealed: for an entry whose page has `meta.encryptHistory`, `history.state` becomes `{ bridge: true, key, scroll, sealed: { v: 1, kid, iv, data } }` where `data` is the ciphertext (an `ArrayBuffer`; history state is structured-cloned) of `{ page, remember }`. Scroll positions and the entry key stay in clear because they carry no application data, which keeps the frequent scroll writes (§11, debounced) free of encryption. Pages without the flag are stored as today.
- Async writes. Web Crypto is asynchronous and the History API is not. Every stored state carries an entry id (`entry`, a per-document prefix plus a counter). `History` keeps the clear state of the current entry in memory while its entry id matches, and every read (`current`, `restore`, used synchronously by `useRemember` and form restore) is served from it. A push or replace of an encrypted page writes a placeholder at once (`{ bridge, key, scroll, entry }`, no page), so the address changes with the render, then replaces it with the sealed state when Web Crypto answers. A seal is dropped when a later write happened or the current entry is no longer the one it was made for (the entry being left can no longer be written, as with scroll today); the entry then keeps its placeholder and back/forward requests it again. `saveScroll` writes the clear `scroll` without encrypting.
- Restore. On `popstate` a sealed entry is decrypted before `restoreFromHistory`; if the key is missing, the key id differs, or decryption fails, the entry is treated as having no page and the router re-requests the URL (the existing path), so the server applies authentication again.
- Clear. A page with `meta.clearHistory` makes the client generate a new key and id, replacing the old one in `sessionStorage` before that page is written to history, then clear the page cache, the once store (§13.2) and the in-memory copies of other entries. Other tabs of the same origin keep their own keys, so the client also increments a `bridge.history.epoch` counter in `localStorage`; each tab stores the epoch with its key and rotates when it sees a newer one (`storage` event, and a check before every decrypt).
- Back-forward cache. A document restored from the back-forward cache keeps its JavaScript state and never fires `popstate`. On `pageshow` with `persisted`, if the current page was encrypted and the stored key id or epoch changed, the client reloads the document.
- No Web Crypto. Web Crypto requires a secure context (HTTPS or `localhost`). Without it an entry that should be encrypted is stored without its page and remembered state (`{ bridge: true, key, scroll }`), so back/forward re-requests it, and one console warning explains why. Plaintext storage is never the fallback.
- Scope. This protects Bridge's own copies. The browser's HTTP cache is covered by `no-store` on authenticated shells (§24); data an application writes elsewhere (its own `localStorage`, service workers) is its responsibility and the docs say so.

- Full reloads (changed 2026-09-24). After a full document load the current entry is sealed and cannot be decrypted synchronously. `Router.init()` keeps the sealed state it replaces, decrypts it, writes remembered keys history does not have yet, and emits `restore` with every decrypted value; `useRemember` and form `remember` (both adapters) apply their value when the user has not changed it since mount. Values therefore arrive a moment after the page appears.

**Tests and docs.** Laravel: config and per-response flags in the page and embedded shell, `clearHistory` in the same response and flashed across a redirect after `session()->invalidate()`, the logout listener, JSON mode unaffected. Core: `tests/historyCipher.test.ts` (round trip, new IV per write, wrong key and tampered ciphertext fail closed), `tests/historyEncryption.test.ts` with a fake `window` (sealed state shape, ordered writes, dropped stale writes, synchronous `restore` from memory, re-request after rotation, epoch rotation from another tab, no-Web-Crypto fallback, `pageshow` reload). Vue and React: `useRemember` and form `remember` restore through a sealed entry. Playground: authenticated routes use `bridge.encrypt-history`; an E2E spec signs in, opens a customer, logs out, goes back, and asserts the customer page is not shown without a request (it lands on the login page). Docs: `docs/security/history-encryption.md`, linked from caching and authentication.
...
- `benchmarks/apps/reference-baseline`: the playground's Customers pages rebuilt with an established server-driven page library (same models, seed, and Vue components), as the reference baseline.
...
| Page JSON vs JSON mode vs baseline page        | k6, 50 VUs, 60 s               | req/s, p95 latency, CPU (`docker stats`), memory                                                                                            |
...
Expected outcome to verify, not assume: Bridge Page ≈ the baseline page in size and latency (the protocol is similar in weight), JSON mode slightly smaller (no `component`/`url`), embedded shell faster to FCP than static-shell mode by one RTT.
...
├── benchmarks/                  k6, Lighthouse CI, phpbench, reference-baseline app, docker-compose, RESULTS.md
...
1. Introduction, and a migration page for developers coming from other server-driven page libraries (what is similar, what is deliberately different).
...
| Deviation from usual semantics confuses adopters (422 vs redirect-back)   | Learning curve           | Explicit comparison page; the difference is one paragraph.                                                             |
...
| 422 in Page mode               | Redirect-back with session flash                 | Session dependency, extra round trip, error bag juggling; breaks stateless clients.                                             |
...
| Separate `core` TS package     | Everything in `vue`                              | Blocks React/RN reuse; a shared core is the proven layout for multi-adapter clients.                                                                        |
...
Since 1.0 (2026-09-24): SSR and the React adapter shipped in Phase 5 (React still labelled experimental); merge props beyond append are §12.1; the mobile SDK guide exists but no SDK; the relay server has a design only; Reverb/Echo transport, GraphQL-like field selection and offline support remain out.
...
- **§10.3, §28 Zero-config page resolution and `withApp` (2026-09-24).** New package `@swarakaka/bridge-vite` (`packages/vite`, in the `fixed` version group), a Vite plugin that rewrites `createBridgeApp(...)` and `createSsrRenderer(...)` calls imported from `@swarakaka/bridge-vue`/`-react` (and their `/server` entries) at build time: a call without `resolve` gets one built on `import.meta.glob('./Pages/**/*.<ext>')` relative to the calling file, and a `pages` option (`'./Dir'` or `{ path, extension, lazy, transform }`) is replaced by the same resolver. It has to be a build step because `import.meta.glob` only works on literal patterns in application source. `resolve` is therefore optional in the adapters' types; without the plugin a missing `resolve` (or a leftover `pages`) throws a message naming the plugin. `withApp(app, { ssr, page })` customises the application before it mounts or renders: the Vue app (register plugins), or for React the element tree (return it wrapped in providers). The same callback can be passed to `createBridgeApp` and `createSsrRenderer`; `setup` (Vue) and `wrap` (React server) remain for full control.

- **§9.2 Application middleware (2026-09-24).** `HandleBridgeRequests` is no longer `final`: applications may extend it and override `share(Request): array`, whose result is shared as request props (dropped after the request, per the §6.5 hardening entry). `php artisan bridge:middleware {name=HandleBridgeRequests}` generates the subclass in `app/Http/Middleware` from `packages/laravel/stubs/middleware.stub`; the app appends it to `web` in `bootstrap/app.php`. Auto-registration skips the package class when the `web` group already contains a subclass (the app's `withMiddleware` callback runs before providers boot), so negotiation never runs twice. Only `share()` is a hook; the build version stays in `bridge.build.version`/`Bridge::setVersion()` and the shell in `bridge.shell.view`.
- **§14 Flat form fields (2026-09-24).** The Vue `useForm` now also exposes each field on the form object (`v-model="form.name"`, `form.items = [...form.items]`); `form.data.*` keeps working and both styles reach the same values (`isDirty`, `reset()`, submission, `remember`). This reverses the Phase 2 choice of `form.data` only; collisions are now handled by refusing them: `useForm` throws when an initial field is named like a `Form` member (public or internal, e.g. `errors`, `data`, `reset`, `progress`, `options`), since a silent shadow would hide either the field or the member. Only the Vue adapter changes: the proxy wraps the reactive form, members (methods bound to the reactive form, `this`-returning methods return the proxy) go to the form, other string keys to `form.data`. The core `Form` API, the protocol and the React adapter (`form.data` with `setData`, since React state cannot be assigned to) are unchanged. `ReactiveForm<T>` is `Form<T> & Omit<T, keyof Form<T>>`: intersecting the class itself, not `UnwrapNestedRefs<Form<T>>`, keeps chained methods (`useForm(...).dontRemember('password')`) typed with the flat fields, because polymorphic `this` resolves to the receiver. Refs inside initial values were never supported (the core clones `initial`).
- **§14 Form helpers (2026-09-24).** `Form` gained `resetAndClearErrors(...keys)` and `dontRemember(...keys)` with `rememberable(values?)`, which the adapters use to write and restore remembered data without those fields, and both adapters accept the remember key first (`useForm('login', data)`). `dontRemember` is top-level only. `defaults()` was not added because `defaults` is already the stored values (a breaking rename); `resetOnError`/`setDefaultsOnSuccess` are left for a form component; an app-wide `recentlySuccessful` duration was not added. React: `useHandle` now returns its proxy from methods that return the handle, so chains keep re-rendering (before, `form.setData(...).post()` called `post` on the raw form and `processing` never rendered), and `dontRemember`/`rememberable` do not re-render, so `useForm(...).dontRemember(...)` can run during render. The design for form-shaped JSON requests is §14.1.
- **§14.1 Form-shaped JSON requests (2026-09-24).** Implemented as designed: `FormState` in core with `Form` and the new `JsonForm` as transports, `bridge.jsonForm()`, and `useJsonForm` in Vue and React. Decisions the design left open: a cancelled or superseded `JsonForm` submission resolves `{ status: 'cancelled' }` (even if the server answered) and calls `onCancel` and `onFinish`; `JsonRequestOptions.mutation: false` keeps the page cache during Precognition; `useJsonForm` cancels on unmount like `useJson`, while `useForm` does not (a page visit outlives its component); both hooks share the `form:` remember namespace. The React docs claim that `useJson` did not exist was stale and was corrected.
- **§14 `setData` callback and the `onError` warning (2026-09-24).** `setData` accepts a callback, `setData((data) => ({ ...data, name }))`, whose result is merged like `setData(values)` (merged rather than replacing `data`, so a partial result keeps the other fields and `setData`'s one rule holds). A `422` reaching a submit or `validate()` that passed `onError` but not `onInvalid` logs one console warning per form (`warnIfOnlyOnError`, a module-level `WeakSet`, so no new form member and no reserved Vue field name), because such a handler silently never runs. Core has no development/production switch; like its other warnings, this one is always on.
- **§15.1 Precognition on forms (2026-09-24).** Added the bound-endpoint style (`useForm(method, url, data)`, `withPrecognition`, `validate(field)`, `validate()`, `submit(options?)`), touched/validated state (`touch`, `touched`, `valid`, `invalid`), a debounce (`setValidationTimeout`) and file exclusion (`validateFiles` to include) on `FormState`, for page and JSON forms and both adapters. `submit` is now defined once on `FormState` and dispatches to the transport's protected `submitTo`. The explicit `validate(method, url, field)` is unchanged (no debounce, files sent). Debounced calls merge their fields instead of keeping only the last call's, so a field requested in the window is never dropped. The playground customer form uses the bound style and shows a "valid" state, covered by `e2e/tests/phase4.spec.ts`.
- **§11, §18 Visit options and cache tags (2026-09-24).** Added `preserveUrl`, `showProgress`, `queryStringArrayFormat` and `invalidateCacheTags` to visits (so to form submissions), `cacheTags` on `prefetch` and `BridgeLink`, and `router.flushByCacheTags`. Two choices keep existing behaviour: arrays in query strings stay `indices` by default (`brackets` is opt-in), and the cache is still cleared after every successful non-GET visit and JSON mutation, so `invalidateCacheTags` adds something only on GET visits and tags otherwise serve manual flushes. `mergeQuery` now deletes a key it writes before appending, which is what allows repeated `a[]` keys and still overrides a key already in the URL. The playground's "load more" uses `preserveUrl` and `showProgress: false` (its E2E now expects `/customers`), and its layout indicator checks `visit.showProgress`. The prefetching page listed the wrong statuses for cache clearing (409 instead of 419) and was corrected.
- **§14.2, §14.3 Planned (2026-09-24).** The form component with `useFormContext` and optimistic updates are designed in §14.2 and §14.3, with open questions for the maintainer.
- **§14.2 Form component (2026-09-24).** Implemented with the proposed defaults (`BridgeForm`, `setDefaultsOnSuccess: true`, no submission without JavaScript). Deviations from the design: reset options moved into core (`SubmitResetOptions`) so field lists reset before new defaults are set; the Vue component rewrites defaults and values after each update because Vue 3.5 re-applies `value` to the attribute too; Vue exposes getters rather than the reactive form; React's context value is a fresh `{ form }` per render so consumers update. §14.3 is still planned.
- **§14.3 Optimistic updates (2026-09-24).** Implemented with the proposed defaults (JSON forms patch page props, public `router.patchProps`, cancelling page visits). Choices the design left open: a `204` success keeps the optimistic values like a JSON success; the page-visit update settles as soon as the page is applied, before `onSuccess`, so callbacks see the server values; `form.optimistic` is one-shot and consumed even when `onBefore` later cancels the submission; a JSON success while another update still holds the same prop does not change the held snapshot, so a later failure of that other update shows the last known server value (reconcile with `patchProps`). `PageStore.replaceProps` and `setProp` now go through the same server-data path.
- **§14 App-wide form defaults (2026-09-24).** `BridgeConfig.forms` (so `createBridgeApp({ forms })` in both adapters) holds `FormOptions` defaults applied by `bridge.form()` and `bridge.jsonForm()`, and therefore by `useForm`, `useJsonForm` and `<BridgeForm>`; a form's own options win. It covers `recentlySuccessfulFor` (the app-wide `recentlySuccessful` duration) and, at no extra cost, the other `FormOptions` (`resetOnSuccess`, `resetOnError`, `setDefaultsOnSuccess`).
- **§13.4, §23.1 Follow-ups (2026-09-24).** (1) Invalidations of an infinite-scroll prop now refresh the loaded range (open question 3 reversed); new public `router.handleInvalidation(prop, handler)` and an `only` option on `router.request()`. (2) Remembered state of an encrypted page survives a full reload through a late restore and the new `restore` router event (`{ values }`); the router writes only keys history lacks, because React's remember writers store the initial value at mount, so a component compares its value with a baseline taken at mount (after chained setters such as `dontRemember()`, which a first version missed). (3) The polling E2E records every appearance of the progress indicator with a `MutationObserver` installed before the page loads, and checks that a real navigation shows it, so the "hidden during polls" assertion can fail. The playground's customer form now uses `remember` (`customer-form:<action>`, without the avatar), covered by a reload E2E in `history.spec.ts`; `infinite-scroll.spec.ts` covers a stream invalidation keeping two loaded pages.
- **§12, `spec/page.md` §3 Partial responses and `meta` (2026-09-24).** `PageStore` replaced the page's `meta` with a partial response's, so a deferred group or a reload that carried any `meta` (on encrypted routes every response carries `encryptHistory`) dropped the per-prop members of props it did not carry: `meta.scroll` (the infinite-scroll bug worked around in its controller), `meta.merge` and the other merge members, and `meta.once`. Partial merges now go through `mergePartialMeta`: per-prop members are updated for the props in the response (including once props listed in its `meta.once`) and kept for the others; page-level members come from the response, as for a full page. The spec states the rule; the scroll controller keeps its copy of `meta.scroll` as a second line of defence.
- **§13.4 Infinite scroll (2026-09-24).** Implemented with the proposed defaults for the open questions (an invalidation replaces the loaded pages; the address follows by default; the playground's customers list uses `<InfiniteScroll>`). Deviations from the design: (1) the address becomes the page just loaded, not "the page nearest the viewport", which would need a mapping from DOM rows to pages the component cannot know; (2) one load at a time in total, not per direction, because a second visit would cancel the first (§11); (3) the manual controls render only when loading is manual (`manual`, `manualAfter` reached, before the component is interactive, including SSR, and without `IntersectionObserver`), not always as visually hidden buttons: keyboard scrolling and tabbing through the items reach the edges, and each load is announced through an `aria-live` region; (4) `reverse` swaps which edge loads which end and anchors next-page loads, and the application renders the items in reverse order. The shared core controller is `pages/infiniteScroll.ts` (`InfiniteScroll`, `readScrollMeta`), constructed without side effects and started after mount; it keeps its ends in history (`bridge:scroll:<prop>`, used only when the list length matches) and keeps the last `meta.scroll` it saw, because any partial response carrying `meta` (a deferred group; on encrypted routes every page carries `meta.encryptHistory`) replaces the page's `meta`. Two fixes came out of the E2E run: the controller bug just described, and `History` now moves the address at once when a sealed page changes its URL, because a following `remember()` write superseded the seal and kept the old address. `Bridge::scroll($value, ?$pageName)` is `Bridge\Props\Scroll` (append merge, default match path `data.id` when every item has an `id`, `->matchOn()` overrides); `meta.scroll` is left out of JSON mode; the schema uses `anyOf` for page values because Ajv's strict mode refuses a three-type union. The package's `json_mode()` test helper now flushes headers like `page()`. The customers list keeps its pagination links; its "load more" and "load previous" buttons are the manual slots; the phase 4 specs scroll instead of clicking, and `e2e/tests/infinite-scroll.spec.ts` covers loading down, the address, opening `?page=3` and prepending page 2 with the viewport kept, and a search resetting the list.
- **§13.3 Combining delivery hints (2026-09-24).** Implemented as designed. Modifiers live on `PropHint` (`merge()`, `append()`, `prepend()`, `deep()`, `matchOn()`, `once()`, `fresh()`), backed by two value objects, `MergeOptions` and `OnceOptions`, and return clones; `Merge` and `Once` set their modifier at construction and keep their constructors and constants. Their public `mode`/`matchOn` and `key`/`ttl`/`fresh` properties (added by §12.1 and §13.2, not yet released) moved to `mergeOptions()`/`onceOptions()`. Choices the design left open: `matchOn()` implies append merge on a hint without a merge mode; `fresh()` without `once()` throws; a held lazy-once or deferred-once prop gets a `meta.once` entry only when held (an entry for an absent, unheld value would make the client request it); a fresh deferred-once prop is deferred again. `spec/page.md` §4.1 has the combinations table; the fixture is `page/deferred-once-held.json`. The package's test helpers `page()`/`html()` now flush headers set by an earlier call in the same test (they accumulated, which hid a partial header leaking into the next request). The playground's `signups` chart is `defer(..., 'charts')->once(ttl: 300)`, covered by an E2E that returns to the dashboard in the app and sees no second `signups` request.
- **§12.1 Merge variants (2026-09-24).** Implemented as designed with the proposed defaults (flat `meta` lists, match paths ending in the item key). Choices the design left open: `Merge` gained `->deep()` besides `Bridge::deepMerge()`, and `->matchOn()` without paths clears matching; `Bridge::merge()` now refuses a hint as its value, like `Bridge::once()`. On the client, `combineProp(current, incoming, mode, matchOn)` and `readMergeModes(page)` live in `pages/PageStore.ts` beside `appendProp` (now a wrapper) so `pages/merge.ts` keeps only the stream `prop` modes; `readMergeKeys` returns keys of every mode. Matching compares item keys with strict equality, keeps the first position of a key already shown, and adds incoming items without the key. Coalesced reloads merge only when every caller passed the same `merge` value, otherwise they replace. The fixture `page/merge-modes.json` is a full page (one key per mode, two match paths). The playground's customers list uses `->matchOn('data.id')`; `e2e/tests/phase4.spec.ts` creates a customer in JSON mode between the first page and "load more" and asserts 39 distinct rows.
- **§12.1, §13.3, §13.4 Planned (2026-09-24).** Merge variants (prepend, deep merge, match paths), combining delivery hints with merge and once modifiers, and infinite scroll are designed in those sections. Proposed order: §12.1 first (smallest; infinite scroll needs prepend and match paths), then §13.3 (mostly server), then §13.4 (largest; needs both). All three are additive within protocol 1 (`meta.prepend`, `meta.deepMerge`, `meta.matchOn`, `meta.scroll`, and a per-visit `merge` direction on the client). Open questions for the maintainer, each with the proposed default: (1) `meta` stays flat lists per mode, with `meta.merge` keeping append keys for old clients, rather than one `meta.merge` object; (2) a match path's last segment is the item key (`data.id` for paginators) rather than a separate "array path" option; (3) an invalidation reload of a scroll prop replaces the loaded pages with the current page (today's replace rule) rather than re-fetching every loaded page; (4) `once` with `merge` is refused rather than defined; (5) infinite scroll replaces the address with the page nearest the viewport by default, with `preserveUrl` to opt out; (6) the playground's customers list moves from the "load more" button to `<InfiniteScroll>`, keeping the button as the manual slot.
- **§23.1 History encryption (2026-09-24).** Implemented as designed, with these choices: an encrypted entry is pushed at once as a placeholder and sealed afterwards, instead of encrypting before `pushState` (the address no longer waits for Web Crypto, so there is no delay to benchmark), and writes are ordered by an entry id and a write counter rather than a promise chain; a pending clear is kept in the session with `put` and consumed (`pull`) by the next page or HTML page, not flashed, so a JSON call or stream connection in between does not lose it; `carryClearHistory` also covers `409` external redirects; `bridge.encrypt-history:false` opts a route out; `page.schema.json` types `meta.merge` together with the two history members (`meta` stays open to other members). `History.open()` decrypts a popped entry (synchronous for plain entries), `History.clear()` rotates the key, and `Router` handles `meta.clearHistory` on every applied response (visits, partial and deferred loads, the booted page) and the `pageshow` check. Remembered state of an encrypted page does not survive a full reload. The playground encrypts its authenticated routes; `e2e/tests/history.spec.ts` covers sealing, restore without a request, and back after signing out.
- **§13.2 Once props (2026-09-24).** Implemented as designed, with these choices: the page cache key leaves `X-Bridge-Once` out (cached pages are completed when applied, missing values fall back to a partial reload); `bridge.cache.clear()` does not clear the once store because the router calls it after every mutation, while `router.clearCache()` clears both; `meta.once` also appears in embedded HTML pages so the client learns the values at boot; a partial response lists only the once props it selected; `->fresh()` returns a copy because shared hints outlive requests. Page responses now vary on `X-Bridge-Once` (`Headers::PAGE_VARY`, protocol `PAGE_VARY`); JSON keeps `VARY`. The resolver's nested-hint check applies to every hint: a hint whose closure returns another hint throws a `LogicException` naming the prop (it used to reach the serializer). New fixtures `page/once-full.json` and `page/once-held.json` (the conformance test constrains its `/customers/{id}` route to numeric ids so `/customers/create` reaches the once route). The playground shares `statuses` between Create and Edit under `customer-statuses`; `e2e/tests/once.spec.ts` checks the header, the omitted value and the rendered options.
- **§11.1 Polling (2026-09-24).** Implemented as designed, plus two choices the design left open: `reload` may be a function read on every tick (adapters use it for the latest options), and polls are background work by default (`showProgress: false`, through a new `ReloadOptions.showProgress`; merged reloads show progress if any caller asked for it). `usePoll` returns `{ start, stop, active }` (`active` a readonly ref in Vue); `stop()` keeps a component's poll stopped when its interval changes. A reload already waiting in the router's debounce window when a poll stops still goes out. The playground polls every 3 s instead of 5 s.
- **§13.1 Load when visible (2026-09-24).** Implemented as designed, except that the observing and loading logic is one core helper, `loadWhenVisible` (`pages/whenVisible.ts`), instead of living in each adapter, so Vue and React cannot drift; the composables add `trackVisibility` (default `true`) so the component can release its observer after loading. The playground's `Customers/Show` loads a lazy `activity` prop inside `<WhenVisible>`. Doing so exposed an existing bug: a partial merge (`PageStore.setPage(..., { partial: true })`, used by deferred loads, invalidations and these loads) cleared an error rendered in place of the page, so a background load finishing after a 403 replaced the error with the page. Partial merges now keep `error`; full page swaps still clear it.
- **§11.1, §13.1, §13.2, §23.1 Planned (2026-09-24).** Polling, load when visible, once props and history encryption were never in the brief or Phases 0–5; the migration guide listed them as missing. They are designed in those sections. Proposed order: history encryption first (a security gap: authenticated props stay readable in `history.state` after logout), then load when visible and polling (client only, no protocol change), then once props (new header, `meta.once`, fixtures on both sides). Encryption and once props are additive within protocol 1 (`meta` members and one request header), so no version bump. Open questions for the maintainer, each with the proposed default: (1) hidden tabs pause polls (proposed) rather than slowing them; (2) without Web Crypto, encrypted entries are stored without their page (proposed, fail closed) rather than in plaintext with a warning; (3) the once store lives in memory per tab (proposed) rather than in `sessionStorage`; (4) the header is named `X-Bridge-Once`; (5) `clear_on_logout` defaults to `true`. When each ships, Appendix B (header, `Vary`) and Appendix C (`history.encrypt`, `history.clear_on_logout`) are updated with the code, and the migration guide's "not yet" line is removed.

### 2.0 (2026-09-24)

- **§9.1–9.2 Facade layout.** `use Bridge\Bridge;` then `Bridge::render()` failed ("Non-static method … cannot be called statically") because `Bridge\Bridge` was the service and the facade lived at `Bridge\Facades\Bridge`. Following the usual Laravel split of a facade in front of a separately named service, the service is now `Bridge\BridgeManager` (singleton, alias `bridge`) and `Bridge\Bridge` is the facade. `Bridge\Facades\Bridge` extends it and is deprecated; the composer `Bridge` alias points at `Bridge\Bridge`. Injecting or resolving `Bridge\Bridge` as the service breaks and gets no shim (a facade instance cannot act as the service), so all packages move to 2.0.0 under the shared version rule (§31); `packages/laravel/UPGRADE.md` describes the migration.

### Hardening (2026-09-24)

- **§20.2 Event bus, `spec/stream.md` §5 Replay.** Buses may implement the optional `Bridge\Stream\Contracts\ReplayWindow` (`canReplayFrom($channels, $lastEventId)`); the stream answers `replayed: false` when it returns false. The database bus refuses ids below its oldest kept row, above its newest, or non-numeric; the Redis bus refuses ids from another driver, ids in the future, and ids older than the oldest entry once a stream has been trimmed (Redis 7+ `entries-added` vs `length`, plus `max-deleted-entry-id` for XDEL). An optional interface rather than a new `EventBus` method keeps custom drivers working.
- **§20.2 Database bus ordering.** Auto-increment ids can become visible out of order on MySQL/PostgreSQL. `Cursor` now carries the connection's `floor` and a bounded list of `recent` ids; the database bus re-reads `stream.drivers.database.lookback` ids (default 200) behind the cursor and returns rows it has not delivered. The stream loop sends an event behind the cursor without an SSE `id` (spec §5 amended) so `Last-Event-ID` never regresses. Publishing after commit (`ShouldDispatchAfterCommit`) is the documented recommendation for transactional work.
- **§6.5 Shared props.** Props shared after the application booted are dropped once each request is handled (`RequestHandled`), so Octane workers cannot carry one user's shared props into another request. Boot-time shares, including the built-in `errors` and `flash`, persist.
- **§24 Caching.** A static shell (`embed(false)`) sent to an authenticated user is `private, max-age=300, must-revalidate`, as `spec/page.md` §7 already required ("MUST NOT emit `public` for an authenticated user"); the table there now lists both rows.
- **Page-mode redirects.** `PageRepresenter::isSameOrigin` treats targets containing `\`, control characters or whitespace, or that `parse_url` rejects, as external (409 + `X-Bridge-Location`), because browsers resolve `/\evil.com` and `/\t/evil.com` to another host.

- **§11 Router concurrency.** One active visit still owns the page, but background work no longer competes for it: coalesced reloads wait for an in-flight navigation or submit (and are dropped when it changed the component), and Precognition validation uses the new `router.request()`, outside the visit pipeline, with one in-flight validation per form. Cached and history restores check that they were not superseded before applying. Scroll positions are saved with a trailing 150 ms debounce while scrolling, because the entry being left cannot be written after `popstate` (and browsers rate-limit `replaceState`). The page cache is cleared by stream invalidations and prop updates and by JSON-mode mutations.
- **§20.5 Stream client, `spec/stream.md` §5–6.** Connection attempts carry a token so `close()` and newer attempts stop stale ones after every `await`; `resolveUrl` failures back off and retry. Resync now follows §6.4: the previous exemption for every orderly `end` hid lost events when the server could not replay; it now applies only when the client had no id to send. To keep routine `max_duration` reconnects gap-free, the server's `end{max_duration}` carries the connection cursor as its `id` when the bus can replay.
- **§20.5 EventSource transport, `spec/stream.md` §5.** A new `EventSource` cannot send `Last-Event-ID`, so the client adds the `lastEventId` query parameter and the server reads it when the header is absent (the header wins; ticket signatures ignore it, as ids grant nothing). `EventSource` also hides the status of a refused connection; the client probes the same URL once with `Accept: application/json`, which the auth middleware answers with 401/403 before the stream route and an allowed request answers with 406, so the probe never opens a stream or takes a connection slot. 401/403 close the stream (spec §6.3); anything else backs off.
...
- **§5.4 `wantsJson()` in page mode.** Page visits reached application code looking like API calls: `Accept: application/vnd.bridge+json` makes Laravel's `wantsJson()`/`expectsJson()` true, so Fortify answered a login with `{"two_factor": false}` (or `204` for the two-factor challenge and logout) instead of a redirect, and the client showed "non-Bridge response". `HandleBridgeRequests` now presents a page visit as a browser navigation (§5.4). `HandleBridgeRequests` was deliberately not moved ahead of `auth` in the middleware priority: `auth` then calls `Authenticate::redirectTo()`, which throws `Route [login] not defined` in apps without that route, while the exception path already yields the same hint and catches that case. Bridge keeps the media type as the only marker (§5.2) and translates on the server, rather than having the client send a browser `Accept` plus a marker header.
- **§31 CI/CD.** Deviates from the original plan: no MySQL service, no playground stream tests per bus (the Pest bus tests cover database and Redis), no PHP-FPM/Nginx or FrankenPHP E2E, no benchmark PR comment (runner numbers are noisy; results are an artifact). Releases are tag-driven: `release.yml` (npm trusted publishing is bound to that file name) runs on a `vX.Y.Z` tag, re-runs CI and E2E on the tagged commit through `workflow_call`, and publishes with provenance; merging to `main` publishes nothing and the changesets "version packages" PR is gone; package `repository` URLs use the canonical lowercase `swarakaka/bridge` so provenance matches; the generated protocol types are drift-checked. Publishing the Laravel package needs a subtree split, since Packagist reads the repository root.
...
- **§38 DoD.** `development/release-readiness.md` tracks each criterion; FrankenPHP in CI and the reference baseline remain open and are excluded from 1.0.
...
- **§10.3 Vue.** Page props are passed to the page component only; layouts receive nothing (they use `usePage()`), because spreading props onto a layout leaked them as DOM attributes. `useForm` exposes `form.data.*` rather than flattened fields to avoid name collisions with form methods (superseded 2026-09-24: flat fields were added, see Post-1.0).
...
| `Last-Event-ID`                           | yes (SSE)     | Replay cursor. Query fallback `lastEventId` for a new native EventSource (spec §5).     | n/a                       |
...
| `Vary`                                   | `Accept, X-Bridge-Only, X-Bridge-Except, X-Bridge-Component` on Page/JSON, plus `X-Bridge-Once` on Page. |
...
return [
    ...
    'cache' => ['etag' => true],
    'history' => ['encrypt' => false, 'clear_on_logout' => true], // §23.1; env BRIDGE_HISTORY_ENCRYPT

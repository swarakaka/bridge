...
### 11.1 Polling (planned, 2026-09-24)
...
- **Core API.** `router.poll(intervalMs, reload?: ReloadOptions, options?: { autoStart?: boolean, keepAlive?: boolean })` returns a handle `{ start(), stop(), active: boolean }`. `autoStart` defaults to `true`. Each tick calls `router.reload(reload)`, so it inherits the existing rules: calls within the debounce window coalesce with other reloads (a stream invalidation and a tick send one request), a reload waits behind a user visit instead of aborting it (hardening note), and a reload whose page component changed meanwhile is cancelled.

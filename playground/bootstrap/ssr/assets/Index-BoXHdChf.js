    ...
    setup(__props) {
        return (_ctx, _push, _parent, _attrs) => {
            if (__props.meta.last_page > 1) {
                            ...
                            ssrRenderComponent(
                                ...
                                },
                                {
                                    default: withCtx((_, _push, _parent, _scopeId) => {
                                        if (_push)
                                            _push(`<span${_scopeId}>${link.label ?? ''}</span>`)
                                        else
                                            return [
                                                createVNode(
                                                    'span',
                ...
                _push(
                    `<!--]--><span class="ml-2 text-slate-500">${ssrInterpolate(__props.meta.from)}–${ssrInterpolate(__props.meta.to)} of ${ssrInterpolate(__props.meta.total)}</span></nav>`,
                )
            } else _push(`<!---->`)
        }
    },
    ...
    setup(__props) {
        ...
        return (_ctx, _push, _parent, _attrs) => {
            ...
            _push(
                ssrRenderComponent(
                    unref(BridgeHead),
                    { title: 'Customers · Bridge' },
                    null,
                    _parent,
                ),
            )
            _push(
                `<div class="flex items-center justify-between"><div><h1 class="text-2xl font-semibold">Customers</h1>`,
            )
            _push(
                ssrRenderComponent(
                    unref(Deferred),
                    { data: 'stats' },
                    {
                        fallback: withCtx((_, _push, _parent, _scopeId) => {
                            if (_push)
                                _push(
                                    `<p class="text-sm text-slate-400"${_scopeId}>Loading stats…</p>`,
                                )
                            else
                                return [
                                    createVNode(
                        ...
                        default: withCtx((_, _push, _parent, _scopeId) => {
                            if (_push)
                                _push(
                                    `<p class="text-sm text-slate-500" data-testid="customer-stats"${_scopeId}>${ssrInterpolate(__props.stats?.total)} total · ${ssrInterpolate(__props.stats?.active)} active </p>`,
                                )
                            else
                                return [
                                    createVNode(
            ...
            _push(
                `</div><input${ssrRenderAttr('value', search.value)} type="search" placeholder="Search name, email or company" class="mt-4 w-full rounded border border-slate-300 px-3 py-2" data-testid="search"><table class="mt-4 w-full text-sm" data-testid="customers-table"><thead class="text-left text-xs uppercase text-slate-500"><tr><th class="py-2">Name</th><th>Email</th><th>Company</th><th>Status</th></tr></thead><tbody class="divide-y divide-slate-200 bg-white"><!--[-->`,
            )
            ssrRenderList(__props.customers.data, (customer) => {
                ...
                if (customer.locked)
                    _push(`<span class="ml-1 text-xs text-amber-600" title="Locked">🔒</span>`)
                else _push(`<!---->`)
                _push(
                    `</td><td>${ssrInterpolate(customer.email)}</td><td>${ssrInterpolate(customer.company)}</td><td><span class="${ssrRenderClass([customer.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600', 'rounded px-2 py-0.5 text-xs'])}">${ssrInterpolate(customer.status)}</span></td></tr>`,
                )
            })
            _push(`<!--]-->`)
            if (__props.customers.data.length === 0)
                _push(
                    `<tr><td colspan="4" class="py-6 text-center text-slate-400" data-testid="empty"> No customers match. </td></tr>`,
                )
            else _push(`<!---->`)
            ...
            if (__props.customers.meta.current_page < __props.customers.meta.last_page)
                _push(
                    `<button type="button" class="rounded border border-slate-300 px-3 py-1 text-sm disabled:opacity-50"${ssrIncludeBooleanAttr(loadingMore.value) ? ' disabled' : ''} data-testid="load-more">${ssrInterpolate(loadingMore.value ? 'Loading…' : `Load more (${__props.customers.data.length} of ${__props.customers.meta.total})`)}</button>`,
                )
            else _push(`<!---->`)
            _push(`</div><!--]-->`)
        }

...
| `<Form>`, `useFormContext()`                                                   | `<BridgeForm>`, `useFormContext()` (see [Form component](/basics/form-component))                                               |
...
**Infinite scroll** is `Bridge::scroll()` with `<InfiniteScroll data="...">` and `useInfiniteScroll()`, with `buffer`, `manual`, `manual-after`, `reverse` and `preserve-url`. The ends travel in `meta.scroll`, and the address follows the page last loaded. See [Infinite scroll](/data/infinite-scroll).
